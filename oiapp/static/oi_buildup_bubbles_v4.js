...
(function () {
  ...
  var cached=null;
  function debug(message, bad) { var el=byId("oi-trend-debug"); if(el){el.textContent="Bubble renderer v4: "+message; el.style.color=bad?"#f87171":"var(--muted)";} }

  function render(data) {
    var chart=byId("oiBuildupTrendChart"), table=byId("oiBuildupTrendTable");
    if (!chart) return;
    if (!window.Plotly) { debug("Plotly did not load.", true); return; }
    ...
    debug("Rendered "+points.length+" day-to-day OI-change bubbles.");
    if (table) {
      var byStrike={};
  ...
  function load() {
    ...
    debug("Loading saved OI-change data…");
    ...
    fetch("/api/oi_buildup_trend?symbol="+encodeURIComponent(symbol)+"&expiration="+encodeURIComponent(expiration)+"&days="+days,{credentials:"same-origin",cache:"no-store"})
      ...
      .catch(function(e){debug(e.message, true);chart.innerHTML='<div style="height:220px;display:grid;place-items:center;color:#f87171">'+e.message+'</div>';});
  }
  function boot() {
    var old=byId("oi-trend-refresh-btn"); if (!old) return;
    debug("Loaded; replacing the legacy chart…");
