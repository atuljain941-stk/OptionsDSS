    ...
    <div class="sh-job" data-key="${job.key}">
      <div class="sh-job-head">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          ${job.is_running ? `<span style="color:#fbbf24;font-weight:800;font-size:11px;display:inline-flex;align-items:center;gap:5px"><span style="width:7px;height:7px;border-radius:50%;background:#fbbf24;display:inline-block;animation:sh-pulse 1.2s infinite"></span>Running now (${Math.round(job.running_seconds)}s)${job.stop_requested ? ' · stopping…' : ''}</span>` : `<span style="font-size:11px;color:${statusColor}">${job.last_run_at ? fmtRelative(job.last_run_at) : 'never run'}</span>`}
          ${job.is_running ? `<button class="sh-btn" data-action="stop-job" data-key="${job.key}" ${job.stop_requested ? 'disabled' : ''} title="Stops at the job's next checkpoint -- not instant, and only takes effect if the job's code checks for it (Signal Notifier's source sweep does).">${job.stop_requested ? '⏳ Stopping…' : '⏹ Stop'}</button>` : ''}
...
function bindEvents() {
  document.body.addEventListener('click', async e => {
    ...
    if (action === 'run-now') {
      ...
      btn.textContent = '⏳';
      try {
        const r = await api(`/scheduler-hub/api/jobs/${key}/stop`, { method: 'POST' });
        if (r.ok === false) alert(r.error || 'Could not stop.');
      } catch (err) {
        alert(`Failed: ${err.message || err}`);
      } finally {
