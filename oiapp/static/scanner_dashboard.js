...
async function loadDashboard(id) {
  ...
  renderTiles();
  await refreshDashboard({ silent: true }).catch(e => { console.warn('initial refresh failed', e); showNote('Dashboard loaded; initial refresh failed: ' + (e.message || e)); });
}
...
async function refreshDashboard({ silent = false } = {}) {
  ...
  renderTiles();
  setBusy(false, `✅ Refreshed ${tiles.length} tile(s)`);
  restartAutoRefresh();
}
...
function restartAutoRefresh() {
  ...
  state.autoTimer = setInterval(() => {
    refreshDashboard({ silent: true }).catch(e => console.warn('auto refresh:', e));
  }, ms);
}
...
function bindTopEvents() {
  ...
  $('sd-auto-refresh')?.addEventListener('change', async () => { syncStateFromUi(); updateSummary(); restartAutoRefresh(); scheduleDashboardSave(); if (state.autoRefresh) await refreshDashboard({ silent: true }).catch(() => null); });
  ...
  $('sd-refresh')?.addEventListener('click', () => runButton('Refresh dashboard', () => refreshDashboard({ silent: false })));
