...
def _fetch_watchlist_sync(watchlist_id=None):
    """Fetch one watchlist's price/volume (+ OI when enabled).

    Was a sequential for-loop, one symbol at a time, with a hardcoded
    time.sleep() between every single one and no timeout at all -- for a
    500+ symbol watchlist, any network slowness could make this run for
    hours with nothing to stop it, silently blocking every other
    watchlist queued behind it in the 7:30 AM pipeline (this function is
    called from a single-threaded scheduler loop that waits for each
    step to finish before moving to the next). Parallelized through the
    shared pool with a bounded deadline, same pattern used everywhere
    else this exact anti-pattern was found.
    """
    ...
    def _fetch_one_sym(sym):
        ok = _fetch_price_only(sym)
        if ok and fetch_oi:
            try:
    ...
    futs = {ex.submit(_fetch_one_sym, sym): sym for sym in syms}
    fetched = []
    for fut, sym in bounded_as_completed(futs, timeout=max(60, len(syms) * 0.5),
            on_timeout=lambda ks: print(f"[scheduler] watchlist '{wl_name}': {len(ks)} symbol(s) "
        ...
        try:
            ...
            print(f"[scheduler] {sym} fetch error: {e}")
...
def _fetch_price_only(sym):
    ...
    try:
        ...
        if is_recently_failed(sym):
            return False  # this symbol failed recently -- skip the network round-trip
        ...
        """, (sym, _pd.Timestamp(latest.name).strftime("%Y-%m-%d"),
