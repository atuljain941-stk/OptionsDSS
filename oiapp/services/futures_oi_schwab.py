    ...
    try:
        rows = con.execute(
            """SELECT contract, MAX(trade_date) AS latest_date, MAX(oi) AS max_oi
               ...
               WHERE LOWER(COALESCE(source,'')) IN ('schwab','cme')
                 AND (UPPER(symbol)=?
                  OR UPPER(symbol)=?
                  OR UPPER(root)=?
                  OR UPPER(root)=?
        ...
        rows = con.execute(
            f"""SELECT trade_date, oi, volume, settle, oi_change, source, fetched_at,
                ...
                WHERE UPPER(contract) IN ({ph})
                  AND LOWER(COALESCE(source,'')) IN ('schwab','cme')
...
def fetch_futures_oi_three_layer(symbol="SPY", *, use_cme_fallback=True, include_cot=True):
    """Fetch futures positioning using the app's three-layer approach.

    Layer 1: Schwab real futures OI snapshots (primary, daily history source).
    ...
    result = fetch_futures_oi_schwab(key)
    layers = [{"name": "schwab", "ok": bool(result.get("ok")), "message": result.get("error", "ok") if not result.get("ok") else "stored Schwab OI rows"}]
