...
def _process_watchlist_schedule_row(row: tuple, today: str, hh_mm: str) -> None:
    ...
    if t_price and not price_ran_today and hh_mm >= t_price:
        try:
            ...
            # Scheduled watchlist refreshes deliberately use yfinance. Broker
            # Greeks/Tastytrade backfills are optional manual enrichment and
            # must never make the scheduler consume the app's worker capacity.
            status, payload = _fetch_data_for_watchlist_core(
                wl_id, source="scheduler", blocking=True, oi_source="yfinance"
            )
    ...
    # Lean-mode policy: the per-watchlist scheduler exists solely for the
    # configured price/OI refresh. Earnings, technical snapshots, SEC data,
    # volume profiles, intraday bars, and all futures OI work remain manual or
    # Scheduler-Hub opt-in jobs so they cannot silently compete with the UI.
    return
