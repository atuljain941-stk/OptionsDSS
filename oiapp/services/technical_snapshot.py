    ...
    try:
        con.execute("""
            CREATE TABLE IF NOT EXISTS technical_snapshot (
                ...
                sr_support      REAL,
                sr_resistance   REAL,
                candle_ctx_score REAL,
                candle_ctx_confluence INTEGER,
                candle_ctx_confluence_total INTEGER,
                computed_at     TEXT NOT NULL,
                PRIMARY KEY (symbol, timeframe, date)
            )
        """)
        try:
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN candle_ctx_score REAL")
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN candle_ctx_confluence INTEGER")
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN candle_ctx_confluence_total INTEGER")
        except Exception:
            pass  # already exist on tables created before this addition
        try:
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN rs_vs_spy REAL")
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN sector_rs REAL")
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN sector_strength REAL")
        except Exception:
            pass  # already exist on tables created before this addition
        try:
            con.execute("ALTER TABLE technical_snapshot ADD COLUMN ema13 REAL")
        except Exception:
            pass  # already exists on tables created before this addition
        # SPY, SPX, and the 11 SPDR sector ETFs need to be genuinely
        # scanned/watched, not just fetched as a side-effect of being a
        # benchmark -- both so RS/SectorRS/SectorStrength reads for
        # OTHER symbols can be backed by a warm cache instead of a live
        # fetch every time, and so these benchmarks get their OWN
        # indicators (RSI, EMA, etc) refreshed on the same cadence as
        # everything else, useful directly (e.g. "is SPY itself
        # overbought right now").
        try:
            benchmark_symbols = ["SPY", "SPX", "XLK", "XLV", "XLF", "XLE", "XLY", "XLP", "XLI", "XLB", "XLU", "XLRE", "XLC"]
            con.executemany("INSERT OR IGNORE INTO symbols (symbol) VALUES (?)", [(s,) for s in benchmark_symbols])
        except Exception:
            pass  # symbols table may not exist yet in some deployments -- non-fatal, watcher just won't include these until it does
...
def compute_technical_snapshot(df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "ema13": round(float(ema13.iloc[-1]), 4),
...
def compute_and_store_for_symbol(symbol: str, timeframe: str = "1d") -> bool:
    ...
    if timeframe == "1d":
        # Score/confluence are only meaningful daily (Candle Context's
        # own candle-detection gate is a daily-bar concept), and only
        # get computed here -- not inside compute_technical_snapshot,
        # which stays a pure price-only function with no symbol/OI
        # dependency. Best-effort: a failure here shouldn't block the
        # rest of the snapshot from being stored.
        try:
            from ..scanners.candle_context_scanner import DEFAULTS as _CCTX_DEFAULTS, _score_symbol_asof, _get_price_history
            full_hist = _get_price_history(symbol, min_days=250)
            if full_hist is not None and len(full_hist["close"]) >= 50:
                result, _gate_passed, raw_score = _score_symbol_asof(symbol, dict(_CCTX_DEFAULTS), full_hist)
                snap["candle_ctx_score"] = raw_score
                if result is not None:
                    conf = result.get("confluence") or {}
                    snap["candle_ctx_confluence"] = conf.get("agreeing")
                    snap["candle_ctx_confluence_total"] = conf.get("total")
        except Exception as e:  # noqa: BLE001
            print(f"[technical_snapshot] candle-context score failed for {symbol}: {type(e).__name__}: {e}")

        # RS vs SPY / SectorRS / SectorStrength -- same formulas
        # RelativeStrength()/SectorRS()/SectorStrength() use at query
        # time, computed once here per symbol per cycle instead of on
        # every single scan/query evaluation. Skipped for the
        # benchmark symbols themselves (SPY/SPX/sector ETFs) -- RS of
        # a thing against itself is meaningless, and SPX has no
        # sector.
        if symbol not in ("SPY", "SPX"):
            try:
                from ..scanners.scanner_builder import _period_return_pct
                from ..services.sector_service import get_symbol_sector, SECTOR_ETFS as _SECTOR_ETFS_MAP
                rs_period = 20

                def _close_col(frame):
                    cols = {c.lower(): c for c in frame.columns}
                    return frame[cols.get("close", "Close")].astype(float)

                stock_ret = _period_return_pct(_close_col(df), rs_period, shift=0)

                spy_df = _history("SPY", "1d")
                spy_ret = _period_return_pct(_close_col(spy_df), rs_period, shift=0) if (spy_df is not None and not spy_df.empty) else None
                if stock_ret is not None and spy_ret is not None:
                    snap["rs_vs_spy"] = round(stock_ret - spy_ret, 2)

                sector = get_symbol_sector(symbol)
                sector_etf = _SECTOR_ETFS_MAP.get(sector) if sector else None
                if sector_etf:
                    sector_df = _history(sector_etf, "1d")
                    if sector_df is not None and not sector_df.empty:
                        sector_ret = _period_return_pct(_close_col(sector_df), rs_period, shift=0)
                        if stock_ret is not None and sector_ret is not None:
                            snap["sector_rs"] = round(stock_ret - sector_ret, 2)
                        if spy_ret is not None and sector_ret is not None:
                            snap["sector_strength"] = round(sector_ret - spy_ret, 2)
            except Exception as e:  # noqa: BLE001
                print(f"[technical_snapshot] RS computation failed for {symbol}: {type(e).__name__}: {e}")
...
def start_technical_snapshot_watcher(interval_seconds: int = 180, batch_symbols: int = 10):
    """This work now happens as step 5 of the 7:30 AM morning_data_pipeline
    job (see scheduled_jobs.py) -- computing all 5 timeframes (1d, 1w,
    1h, 2h, 4h) for every symbol once, for the current date only, rather
    than continuously cycling small batches through the day. Still
    registered here (job_registry only, no unified_scheduler dispatch)
    for Scheduler Hub visibility and a manual "Run Now" for an ad-hoc
    recompute (a full batch across all 5 timeframes) outside the 7:30
    window.
    """
    ...
    def _run_full_batch_now():
        from ..scanners.scanner_builder import get_all_cached_daily_symbols
        symbols = get_all_cached_daily_symbols()
        return run_technical_snapshot_batch(symbols, timeframes=["1d", "1w", "1h", "2h", "4h"])

    register_job(
        "technical_snapshot_cache", "Technical indicator precompute cache (runs at 7:30 AM)",
        "Precomputes RSI14/RSI3/EMA-RSI-90/rsidiff90/EMA9-20-50-60-200/MACD/ADX/DI+-/S-R "
        "for every watchlist symbol, across daily/weekly/1h/2h/4h (5 rows per symbol per "
        "day), so scanners and scoring read a cached value instead of recomputing from raw "
        "price history every time. Runs automatically as part of the 7:30 AM morning data "
        "pipeline; Run Now here triggers a full ad-hoc recompute across all 5 timeframes.",
        kind="interval", default_schedule={"interval_min": max(1, int(interval_seconds / 60))},
        group="Manual / One-Time",
        run_now_fn=_run_full_batch_now,
    )
    return True
...
def api_bulk_compute():
    ...
    # Incremental by default: skip symbol+timeframe pairs already
    # computed today -- clicking "Compute Indicators" repeatedly (or
    # this running as part of a routine refresh) shouldn't redo work
    # the background watcher or a prior run already did. force=true
    # bypasses this for a genuine full recompute.
    today = datetime.now().date().isoformat()
    work_items = [(s, tf) for s in symbols for tf in timeframes]
    ...
    if not force:
        try:
            ...
            rows = con.execute(
                f"SELECT symbol, timeframe FROM technical_snapshot WHERE date=? AND symbol IN ({placeholders})",
                [today] + symbols,
            ).fetchall()
            con.close()
            already_done = {(r[0], r[1]) for r in rows}
        except Exception:
            already_done = set()
    todo = [(s, tf) for (s, tf) in work_items if (s, tf) not in already_done]

    def _run():
        global _bulk_status
        with _bulk_lock:
            _bulk_status = {"running": True, "processed": 0, "total": len(work_items),
                             "succeeded": len(already_done & set(work_items)), "failed": 0,
                             "skipped_already_current": len(already_done & set(work_items))}
