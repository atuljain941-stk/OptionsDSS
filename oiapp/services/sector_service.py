...
_sector_perf_cache = {"ts": 0.0, "data": None}
...
def get_sector_performance():
    ...
    Cached for _SECTOR_PERF_TTL seconds, and the 11 per-ETF yfinance calls
    ...
    """
    now = time.time()
    if _sector_perf_cache["data"] is not None and (now - _sector_perf_cache["ts"]) < _SECTOR_PERF_TTL:
    ...
    _sector_perf_cache["data"] = results
    return results
