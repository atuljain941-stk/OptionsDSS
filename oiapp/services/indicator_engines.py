...
class TechnicalSeriesEngine:
    ...
    @staticmethod
    def _rsi_diff_90_colored(rsi_diff_90: pd.Series, rsi14: pd.Series, rsi_ema90: pd.Series,
                              close: pd.Series, ema20: pd.Series, ema50: pd.Series) -> List[Dict]:
        """Exact port of the reference Pine Script's rsi2color ladder
        (user-provided indicator source), NOT a re-derived
        approximation -- price context (close vs ema20/ema50), not just
        the RSIDiff90 magnitude, decides the color:

          rsi14 > rsi_ema90+20 and close>ema50  -> red
          rsi14 > rsi_ema90+8  and close>ema50  -> blue
          rsi14 > rsi_ema90+8  and close<=ema50 -> orange
          rsi14 > rsi_ema90+5  and close>ema20  -> yellow
          rsi14 < rsi_ema90-20 and close<ema50  -> red
          rsi14 < rsi_ema90-8  and close<ema50  -> blue
          rsi14 < rsi_ema90-8  and close>=ema50 -> orange
          rsi14 < rsi_ema90-5  and close<ema20  -> yellow
          else -> gray (Pine's default is white; substituted here since
                  literal white is invisible on a light background and
                  blends into a dark one -- a disclosed visibility fix,
                  not a formula change)

        Conditions checked via np.select in the EXACT same order Pine's
        nested ternary evaluates them (first match wins), not a set of
        independently-evaluated boolean checks that could disagree with
        Pine's actual precedence on an overlapping case.
        """
        conditions = [
            (rsi14 > (rsi_ema90 + 20)) & (close > ema50),
            (rsi14 > (rsi_ema90 + 8)) & (close > ema50),
            (rsi14 > (rsi_ema90 + 8)) & (close <= ema50),
            (rsi14 > (rsi_ema90 + 5)) & (close > ema20),
            (rsi14 < (rsi_ema90 - 20)) & (close < ema50),
            (rsi14 < (rsi_ema90 - 8)) & (close < ema50),
            (rsi14 < (rsi_ema90 - 8)) & (close >= ema50),
            (rsi14 < (rsi_ema90 - 5)) & (close < ema20),
        ]
        choices = ["red", "blue", "orange", "yellow", "red", "blue", "orange", "yellow"]
        colors = np.select(conditions, choices, default="gray")
        out = []
        for ts, v, c in zip(rsi_diff_90.index, rsi_diff_90.values, colors):
            if v is None or (isinstance(v, float) and math.isnan(v)):
                continue
            out.append({"time": int(ts.timestamp()), "value": round(float(v), 4), "color": str(c)})
        return out
    ...
    def compute(self, df: pd.DataFrame) -> Dict:
        ...
        close = df["close"]
        rsi3 = self._rsi(close, 3)
        rsi14 = self._rsi(close, 14)
        rsi_ema13 = self._ema(rsi14, 13)   # "rsiemalength1" in the reference Pine script
        rsi_ema90 = self._ema(rsi14, 90)   # "rsiemalength2" in the reference Pine script
        ...
        return {
            ...
            "ema13": self._to_series(self._ema(close, 13)),
            "ema20": self._to_series(ema20),
            "ema50": self._to_series(ema50),
            "ema200": self._to_series(self._ema(close, 200)),
            "rsi3": self._to_series(rsi3),
            "rsi14": self._to_series(rsi14),
            "rsi_ema13": self._to_series(rsi_ema13),
            "rsi_ema90": self._to_series(rsi_ema90),
            "rsi_diff_90": self._to_series(rsi_diff_90),
            "rsi_diff_90_colored": self._rsi_diff_90_colored(rsi_diff_90, rsi14, rsi_ema90, close, ema20, ema50),
