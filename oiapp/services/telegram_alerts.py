    ...
    try:
        for row in rows:
            ...
            if crossing and (direction == "both" or direction == crossing):
                ...
                # Queue instead of sending inline: this check loop shouldn't
                # block on a Telegram API call (or retry) for every symbol
                # that crosses its level -- queue_alert() is a fast DB write
                # and returns immediately; Signal Notifier's loop delivers
                # it via process_due_alerts() moments later. The crossing
                # itself (not delivery success) is what's recorded below, so
                # a slow/failed Telegram call never causes the same
                # crossing to be detected and queued again.
                from .alert_outbox import queue_alert
                queue_alert(message, channel="telegram", source="telegram_price_alert",
                            payload={"symbol": symbol, "watchlist_id": row["watchlist_id"], "crossing": crossing})
                sent += 1
                con.execute(
...
def _run_once_for_hub():
    """Single-iteration body registered with the unified scheduler hub
    (see services/unified_scheduler.py) -- runs a price-alert check and
    returns a short status string for job_registry's last-run note."""
    result = check_watchlist_price_alerts()
    return f"checked={result.get('checked')} sent={result.get('sent')}"
...
def start_telegram_alert_watcher(interval_seconds: int = 60):
    ...
    with _WATCHER_LOCK:
        ...
        try:
            default_interval = get_global_alert_interval_seconds(interval_seconds or 60)
        except Exception:
            default_interval = int(interval_seconds or 60)
        register_job(
            "telegram_price_alerts", "Telegram price alerts (via Signal Notifier)",
            "Checks watchlist price alerts and pushes Telegram messages. Runs "
            "automatically from Signal Notifier's background loop; configure "
            "enabled/interval at /signal-notifier. Run Now here triggers an extra "
            "ad-hoc check.",
            kind="interval", default_schedule={"interval_min": max(1, int((default_interval or 60) / 60))},
            group="Manual / One-Time", run_now_fn=check_watchlist_price_alerts,
        )
