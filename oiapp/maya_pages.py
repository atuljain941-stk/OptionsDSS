...
def _iv_proxy(df: pd.DataFrame) -> Optional[int]:
    ...
    if len(close) < 30:
        return None
    # Guarding close.iloc[i-1]>0 alone doesn't catch a NaN at close.iloc[i]
    # itself (yfinance data gap) -- that still produces math.log(NaN) and
    # crashes the round()/int() below with "cannot convert float NaN to
    # integer". Require close.iloc[i] itself finite and positive too --
    # same fix applied to the duplicated copies of this calc elsewhere
    # (routes_strategy.py, opportunity_scanner.py, trade_opportunity_scanner.py,
    # spy_strategies.py).
    rets30 = [math.log(close.iloc[i] / close.iloc[i - 1]) for i in range(max(1, len(close) - 29), len(close)) if close.iloc[i - 1] > 0 and math.isfinite(close.iloc[i]) and close.iloc[i] > 0]
    rets90 = [math.log(close.iloc[i] / close.iloc[i - 1]) for i in range(max(1, len(close) - 89), len(close)) if close.iloc[i - 1] > 0 and math.isfinite(close.iloc[i]) and close.iloc[i] > 0] if len(close) >= 90 else rets30
    ...
    if not rv90:
        return None
    _ivr_raw = (rv30 / rv90) * 50
    if not math.isfinite(_ivr_raw):
        return None
    return int(max(0, min(100, round(_ivr_raw))))
...
def api_scanner():
    ...
    try:
        ...
        for fut, sym in bounded_as_completed(futs, timeout=60,
                on_timeout=lambda ks: print(f"[maya_pages] {len(ks)} symbol(s) timed out: {ks[:20]}")):
            if fut is None:
