...
def _adaptive_volume_profile(df: pd.DataFrame, spot: float, windows: Tuple[int, ...] = (63, 126, 252), bins: int = 72) -> Dict[str, Any]:
    """Pick the most actionable profile window near current price.

    We score candidate lookbacks by how close the profile POC sits to spot,
    while preferring windows that still have enough history to be stable.
    """
    ...
    for lookback in windows:
        ...
        vp = _volume_profile(sub, bins=bins)
        poc = vp.get("poc")
        if poc is None:
            continue
        dist = abs(float(poc) - float(spot)) / max(1e-9, float(spot))
        vp["lookback"] = int(len(sub))
        vp["anchor"] = f"last {len(sub)} bars"
        candidates.append((dist, -len(sub), vp))

    if not candidates:
        vp = _volume_profile(df.tail(252), bins=bins)
        vp["lookback"] = int(min(len(df), 252)) if df is not None else 0
        vp["anchor"] = f"last {vp.get('lookback', 0)} bars"
        return vp

    candidates.sort(key=lambda x: (x[0], x[1]))
    best = candidates[0][2]
    best["profile_candidates"] = [
        {"lookback": c[2].get("lookback"), "anchor": c[2].get("anchor"), "poc": c[2].get("poc"), "vah": c[2].get("vah"), "val": c[2].get("val")}
        for c in candidates
    ]
    return best
...
def _analyze_symbol(sym: str, benchmark: str = "SPY") -> Dict[str, Any]:
    ...
    # Volume profile on daily data (adaptive profile near current price)
    spot = float(daily["Close"].iloc[-1])
    vp = _adaptive_volume_profile(daily, spot, windows=(63, 126, 252), bins=72)
    ...
    summary_thesis = (
        ...
        f"Profile anchor: {vp.get('anchor', 'n/a')}. "
    ...
    return {
        ...
        "volume_profile": {
            **vp,
            ...
            "profile_window": vp.get("lookback"),
            "profile_anchor": vp.get("anchor"),
...
def scan():
    ...
    try:
        ...
        for fut, sym in bounded_as_completed(futures, timeout=60,
                on_timeout=lambda ks: print(f"[market_structure] {len(ks)} symbol(s) timed out: {ks[:20]}")):
            if fut is None:
