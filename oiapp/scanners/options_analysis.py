...
def momentum_backtest():
    ...
    try:
        ...
        rows = []
        ex = ThreadPoolExecutor(max_workers=min(10, max(2, len(symbols))))
        try:
            futs = {ex.submit(_build_trade, sym, lookback): (sym, lookback) for lookback in run_offsets for sym in symbols}
            from ..services.bounded_wait import bounded_as_completed
            for fut, key in bounded_as_completed(futs, timeout=90,
                    on_timeout=lambda ks: print(f"[options_analysis] {len(ks)} symbol/lookback pair(s) timed out: {ks[:20]}")):
                if fut is None:
                    sym, lookback = key
                    rows.append({"symbol": sym, "lookback_days": lookback, "error": "timed out"})
                    continue
...
def uae_watchlist_backtest():
    ...
    try:
        ...
        try:
            ...
            from ..services.bounded_wait import bounded_as_completed
            for fut, s in bounded_as_completed(futs, timeout=90,
                    on_timeout=lambda ks: print(f"[options_analysis] {len(ks)} symbol(s) timed out: {ks[:20]}")):
                if fut is None:
                    symbol_results.append({"symbol": s, "error": "timed out"})
                    continue
