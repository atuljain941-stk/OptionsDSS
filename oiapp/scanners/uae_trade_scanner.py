...
def _yf_history(symbol: str, tf: str):
    ...
    if tf == "1wk":
        return _normalise_ohlcv(get_history_cached(sym, period="5y", interval="1wk"))
    if tf == "1d":
        return _normalise_ohlcv(get_history_cached(sym, period="2y", interval="1d"))
    if tf == "4h":
        # Yahoo has no stable 4h interval. Resample recent 1h bars.
        one_h = _normalise_ohlcv(get_history_cached(sym, period="6mo", interval="1h"))
        return _resample_ohlcv(one_h, "4h")
    if tf == "1h":
        return _normalise_ohlcv(get_history_cached(sym, period="6mo", interval="1h"))
    if tf == "15m":
        return _normalise_ohlcv(get_history_cached(sym, period="30d", interval="15m"))
    if tf == "5m":
        return _normalise_ohlcv(get_history_cached(sym, period="10d", interval="5m"))
    return _normalise_ohlcv(get_history_cached(sym, period="1y", interval="1d"))
...
def _hist_iv_proxy(symbol: str) -> float:
    ...
    try:
        from ..services.market import get_history_cached
        df = get_history_cached(sym, period="3mo", interval="1d")
