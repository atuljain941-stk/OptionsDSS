...
def _get_ta(symbol: str, prefetched_df=None):
    ...
    try:
        ...
        # Same NaN-guard fix as the duplicated copies of this calc in
        # routes_strategy.py/opportunity_scanner.py/spy_strategies.py/maya_pages.py --
        # C[i-1]>0 alone doesn't catch a NaN at C[i] itself.
        rets30 = [_m.log(C[i]/C[i-1]) for i in range(n-29, n+1) if C[i-1] > 0 and _m.isfinite(C[i]) and C[i] > 0]
        rets90 = [_m.log(C[i]/C[i-1]) for i in range(n-89, n+1) if C[i-1] > 0 and _m.isfinite(C[i]) and C[i] > 0] if n >= 90 else rets30
        ...
        ivr    = min(100, max(0, round(_ivr_raw))) if _ivr_raw is not None and _m.isfinite(_ivr_raw) else 40
...
def _get_earn_info(symbol: str) -> Dict[str, Any]:
    """Same lookup as _get_earn_days, but keeps the actual earnings DATE
    too (get_earnings_info already returns it -- _get_earn_days just
    threw it away). Needed for both showing the date in alerts and
    checking whether an expiry falls on/after it."""
    try:
        from .earnings_calendar import get_earnings_info
        info = get_earnings_info(symbol) or {}
        days = info.get("earn_days")
        return {"earn_days": int(days) if days is not None else 999, "earn_date": info.get("earn_date")}
    except Exception:
        return {"earn_days": 999, "earn_date": None}
...
def _entry_score(
    ...
    rsi_diff: Optional[float] = None, max_pain: Optional[float] = None,
    dte: Optional[int] = None, regime_detail: str = "",
    ...
    params (new): every constant below now comes from
    scoring_params.get_params() by default -- pass an explicit params
    dict (e.g. a hypothetical edited version from the Scoring Parameters
    page) to score against THAT instead of the currently-saved live
    values, which is what the "simulate" / live re-scan comparison
    feature uses. Omit it and this behaves exactly as before, just
    reading its constants from the DB-backed config instead of literals.

    confluence/rsi_trend: daily regime alone can rate a directional trade
    highly even when the weekly timeframe doesn't support it, or when RSI
    is actively moving the opposite direction of the proposed bias.

    rsi_diff/max_pain/dte/regime_detail: rsi_diff (RSIdiff90) is a scored
    input in its own right, AND used to dampen/nullify the RS-vs-SPY
    bonus when they disagree. regime_detail is the graduated
    regime_scan.regime column (e.g. TRENDING_UP / MILD_BULLISH), not just
    the flat bias. RS's contribution is also damped when regime_detail
    already confirms the same direction strongly. PCR's directional read
    follows this app's dealer-footprint framing (GEX Plan, Wall Term
    Structure) rather than retail-sentiment theory -- a stated modeling
    choice, not settled options theory. max_pain is deliberately
    lightweight. gamma_flip's weight scales down as dte grows, since
    dealer gamma pinning is a same-day-to-few-day mechanic.
    """
    if params is None:
    ...
    # -- Regime (graduated, not flat "contains bull/bear") --------------------
    rd = (regime_detail or "").upper().strip()
    b = (regime_bias or "").lower()
    regime_pts = p["regime_points"].get(rd)
    if regime_pts is None:
        if "bull" in b:  regime_pts = p["regime_flat_fallback_bull"]
        elif "bear" in b: regime_pts = p["regime_flat_fallback_bear"]
        else: regime_pts = 0

    regime_agrees_bull = regime_pts > 0
    regime_agrees_bear = regime_pts < 0
    rsi_pen = p["rsi_trend_against_penalty"]

    if is_bull:
        score += regime_pts
        if regime_pts > 0:
            pros.append(f"Regime {rd or 'bullish'} ✓ ({regime_pts:+g})")
        elif regime_pts < 0:
            cons.append(f"Regime {rd or 'bearish'} — headwind ({regime_pts:+g})")
        if rsi_trend == "FALLING":
            score -= rsi_pen; cons.append("RSI trending down — momentum fading against this bullish call")
    elif is_bear:
        score += -regime_pts
        if regime_pts < 0:
            pros.append(f"Regime {rd or 'bearish'} ✓ ({-regime_pts:+g})")
        elif regime_pts > 0:
            cons.append(f"Regime {rd or 'bullish'} — headwind ({-regime_pts:+g})")
        if rsi_trend == "RISING":
            score -= rsi_pen; cons.append("RSI trending up — momentum improving against this bearish call")

    # Multi-timeframe confluence
    if not is_ic:
        if confluence == "AGREE":
            score += p["confluence_agree"]; pros.append("Daily + weekly trend agree")
        elif confluence == "WEEKLY_SHOCK_AGAINST":
            score += p["confluence_weekly_shock_against"]; cons.append("Recent weekly candle moved sharply against this trade's direction — possible reversal in progress")
        elif confluence == "DISAGREE":
            score += p["confluence_disagree"]; cons.append("Daily and weekly trend disagree — this call is fighting the higher timeframe")
        elif confluence == "WEEKLY_SIDEWAYS":
            score += p["confluence_weekly_sideways_directional"]; cons.append("Weekly is sideways — consider an Iron Condor instead of a directional call")
    elif is_ic and confluence == "WEEKLY_SIDEWAYS":
        score += p["confluence_weekly_sideways_ic"]; pros.append("Weekly genuinely range-bound ✓ — fits an Iron Condor well")
    elif is_ic:
        if "bull" in b or "bear" in b:
            score += p["confluence_ic_directional_regime"]; pros.append(f"Directional regime — IC acceptable")

    # RS vs SPY (damped by momentum conflict and regime redundancy)
    if rs is not None:
        raw_rs_pts = 0.0
        rs_note = ""
        if is_bull:
            if rs > p["rs_bull_strong_threshold"]:    raw_rs_pts, rs_note = p["rs_bull_strong"], f"RS +{rs:.1f}% vs SPY"
            elif rs > p["rs_bull_mild_threshold"]:  raw_rs_pts, rs_note = p["rs_bull_mild"],  f"RS +{rs:.1f}% vs SPY (mild)"
            elif rs < -p["rs_bull_strong_neg_threshold"]: raw_rs_pts, rs_note = p["rs_bull_strong_neg"], f"RS {rs:.1f}% vs SPY -- underperforming"
            elif rs < -p["rs_bull_weak_neg_threshold"]: raw_rs_pts, rs_note = p["rs_bull_weak_neg"],  f"RS {rs:.1f}% vs SPY (mild weakness)"
        elif is_bear:
            if rs < -p["rs_bull_strong_threshold"]:   raw_rs_pts, rs_note = p["rs_bull_strong"], f"RS {rs:.1f}% vs SPY -- confirmed weakness"
            elif rs < -p["rs_bull_mild_threshold"]: raw_rs_pts, rs_note = p["rs_bull_mild"],  f"RS {rs:.1f}% vs SPY (mild weakness)"
            elif rs > p["rs_bull_strong_neg_threshold"]:  raw_rs_pts, rs_note = p["rs_bull_strong_neg"], f"RS +{rs:.1f}% -- stock outperforming"
            elif rs > p["rs_bull_weak_neg_threshold"]:  raw_rs_pts, rs_note = p["rs_bull_weak_neg"],  f"RS +{rs:.1f}% vs SPY"
        elif is_ic:
            if abs(rs) <= p["rs_ic_neutral_threshold"]: raw_rs_pts, rs_note = p["rs_ic_neutral"], f"RS {rs:+.1f}% neutral -- ideal for IC"
            elif abs(rs) > p["rs_ic_trending_threshold"]: raw_rs_pts, rs_note = p["rs_ic_trending_penalty"], f"RS {rs:+.1f}% -- trending hard, IC risky"

        rs_pts = raw_rs_pts
        if raw_rs_pts != 0 and not is_ic:
            if rsi_diff is not None:
                bull_rs_bear_mom = is_bull and raw_rs_pts > 0 and rsi_diff < 0
                bear_rs_bull_mom = is_bear and raw_rs_pts > 0 and rsi_diff > 0
                if bull_rs_bear_mom or bear_rs_bull_mom:
                    zero_t = p["rs_momentum_conflict_zero_threshold"]
                    if rsi_diff < -zero_t or rsi_diff > zero_t:
                        rs_pts = 0.0
                        cons.append(f"RS looks favorable but RSIdiff90 {rsi_diff:+.1f} disagrees — "
                                    f"RS bonus nullified, momentum contradicts the relative-strength read")
                    else:
                        rs_pts = raw_rs_pts * p["rs_momentum_conflict_damp_factor"]
                        cons.append(f"RS favorable but RSIdiff90 {rsi_diff:+.1f} mildly disagrees — RS credit reduced")
            if rs_pts > 0 and ((is_bull and regime_agrees_bull) or (is_bear and regime_agrees_bear)) and abs(regime_pts) >= p["rs_regime_redundancy_min_regime_pts"]:
                rs_pts = rs_pts * p["rs_regime_redundancy_damp_factor"]
                rs_note += " (reduced -- regime already confirms this)"
        if rs_pts != 0:
            score += rs_pts
            (pros if rs_pts > 0 else cons).append(rs_note)

    # RSIdiff90 momentum (direct scored input)
    if rsi_diff is not None:
        st = p["rsi_diff_strong_threshold"]
        if is_bull:
            if rsi_diff > st:   score += p["rsi_diff_strong_bonus"]; pros.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum confirms")
            elif rsi_diff > 0:  score += p["rsi_diff_mild_bonus"]; pros.append(f"RSIdiff90 {rsi_diff:+.1f} — mild momentum support")
            elif rsi_diff < -st: score -= p["rsi_diff_strong_penalty"]; cons.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum meaningfully against this bullish trade")
            elif rsi_diff < 0:  score -= p["rsi_diff_mild_penalty"]; cons.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum mildly against")
        elif is_bear:
            if rsi_diff < -st:  score += p["rsi_diff_strong_bonus"]; pros.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum confirms")
            elif rsi_diff < 0:  score += p["rsi_diff_mild_bonus"]; pros.append(f"RSIdiff90 {rsi_diff:+.1f} — mild momentum support")
            elif rsi_diff > st: score -= p["rsi_diff_strong_penalty"]; cons.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum meaningfully against this bearish trade")
            elif rsi_diff > 0:  score -= p["rsi_diff_mild_penalty"]; cons.append(f"RSIdiff90 {rsi_diff:+.1f} — momentum mildly against")

    # IV Rank
    if iv_rank is not None:
        if is_credit:
            if iv_rank > p["iv_rank_credit_high_threshold"]:   score += p["iv_rank_credit_high"]; pros.append(f"IVR {iv_rank:.0f}% — premium rich")
            elif iv_rank > p["iv_rank_credit_good_threshold"]: score += p["iv_rank_credit_good"];  pros.append(f"IVR {iv_rank:.0f}% — good credit")
            elif iv_rank < p["iv_rank_credit_low_threshold"]: score += p["iv_rank_credit_low"]; cons.append(f"IVR {iv_rank:.0f}% — thin premium")
            elif iv_rank < p["iv_rank_credit_thin_threshold"]: score += p["iv_rank_credit_thin"];  cons.append(f"IVR {iv_rank:.0f}% — low IV")
        else:
            if iv_rank < p["iv_rank_debit_cheap_threshold"]:   score += p["iv_rank_debit_cheap"]; pros.append(f"IVR {iv_rank:.0f}% — options cheap")
            elif iv_rank < p["iv_rank_debit_reasonable_threshold"]: score += p["iv_rank_debit_reasonable"];  pros.append(f"IVR {iv_rank:.0f}% — reasonable cost")
            elif iv_rank > p["iv_rank_debit_expensive_threshold"]: score += p["iv_rank_debit_expensive"]; cons.append(f"IVR {iv_rank:.0f}% — options expensive")

    # PCR -- dealer-footprint framing (see module docstring)
    if pcr is not None:
        if is_bull:
            if pcr > p["pcr_strong_threshold"]:   score += p["pcr_strong"]; pros.append(f"PCR {pcr:.2f} — heavy put buying, contrarian bullish")
            elif pcr > p["pcr_mild_threshold"]: score += p["pcr_mild"];  pros.append(f"PCR {pcr:.2f} — put-leaning, mildly bullish")
            elif pcr < p["pcr_strong_against_threshold"]: score += p["pcr_strong_against"]; cons.append(f"PCR {pcr:.2f} — heavy call buying (retail chasing), contrarian bearish")
            elif pcr < p["pcr_mild_against_threshold"]: score += p["pcr_mild_against"];  cons.append(f"PCR {pcr:.2f} — call-leaning, mildly bearish")
        elif is_bear:
            if pcr < p["pcr_strong_against_threshold"]:   score += p["pcr_strong"]; pros.append(f"PCR {pcr:.2f} — heavy call buying, contrarian bearish")
            elif pcr < p["pcr_mild_against_threshold"]: score += p["pcr_mild"];  pros.append(f"PCR {pcr:.2f} — call-leaning, mildly bearish")
            elif pcr > p["pcr_strong_threshold"]:  score += p["pcr_strong_against"]; cons.append(f"PCR {pcr:.2f} — heavy put buying, contrarian bullish")
        elif is_ic:
            if p["pcr_ic_low"] < pcr < p["pcr_ic_high"]: score += p["pcr_ic_balanced"]; pros.append(f"PCR {pcr:.2f} — balanced (IC ideal)")

    # Max pain
    if spot and max_pain and spot > 0:
        mp_dist_pct = (max_pain - spot) / spot * 100
        fav_t, against_t = p["max_pain_favor_threshold_pct"], p["max_pain_against_threshold_pct"]
        if is_bull and mp_dist_pct > fav_t:
            score += p["max_pain_bonus"]; pros.append(f"Max pain ${max_pain:.0f} sits above spot — mild pull in this trade's favor")
        elif is_bull and mp_dist_pct < -against_t:
            score += p["max_pain_penalty"]; cons.append(f"Max pain ${max_pain:.0f} sits well below spot — mild pull against this trade")
        elif is_bear and mp_dist_pct < -fav_t:
            score += p["max_pain_bonus"]; pros.append(f"Max pain ${max_pain:.0f} sits below spot — mild pull in this trade's favor")
        elif is_bear and mp_dist_pct > against_t:
            score += p["max_pain_penalty"]; cons.append(f"Max pain ${max_pain:.0f} sits well above spot — mild pull against this trade")

    # Wall support/resistance
    if spot and put_wall and call_wall:
        pd = (spot - put_wall) / spot * 100
        cd = (call_wall - spot) / spot * 100
        strong_t, mild_t = p["wall_strong_threshold_pct"], p["wall_mild_threshold_pct"]
        if is_bull:
            if pd < strong_t:    score += p["wall_strong"]; pros.append(f"Put wall ${put_wall:.0f} — strong support nearby")
            elif pd < mild_t:  score += p["wall_mild"];  pros.append(f"Put wall ${put_wall:.0f} ({pd:.1f}% away)")
            else:         score += p["wall_far_penalty"];  cons.append(f"Put wall ${put_wall:.0f} far ({pd:.1f}%)")
        elif is_bear:
            if cd < strong_t:    score += p["wall_strong"]; pros.append(f"Call wall ${call_wall:.0f} — strong resistance nearby")
            elif cd < mild_t:  score += p["wall_mild"];  pros.append(f"Call wall ${call_wall:.0f} ({cd:.1f}% away)")
            else:         score += p["wall_far_penalty"];  cons.append(f"Call wall ${call_wall:.0f} far ({cd:.1f}%)")
        elif is_ic:
            pinched_t, risk_t = p["wall_ic_pinched_threshold_pct"], p["wall_ic_risk_threshold_pct"]
            if pd < pinched_t and cd < pinched_t:
                score += p["wall_ic_pinched"]; pros.append(f"Pinched walls ${put_wall:.0f}-${call_wall:.0f}")
            elif cd < risk_t or pd < risk_t:
                score += p["wall_ic_risk_penalty"];  cons.append("Spot too close to one wall — IC risk")

    # Gamma flip -- weight scales down as dte grows
    if spot and gamma_flip:
        if dte is None:
            gamma_scale = 1.0
        elif dte <= p["gamma_flip_dte_scale_full"]:
            gamma_scale = 1.0
        elif dte <= p["gamma_flip_dte_scale_half"]:
            gamma_scale = 0.5
        elif dte <= p["gamma_flip_dte_scale_quarter"]:
            gamma_scale = 0.25
        else:
            gamma_scale = 0.1
        gf_pts = round(p["gamma_flip_base_points"] * gamma_scale)
        gf_penalty = round(p["gamma_flip_near_penalty"] * gamma_scale)
        near_t = p["gamma_flip_near_threshold_pct"] / 100.0
        if is_bull and spot > gamma_flip:
            if gf_pts: score += gf_pts; pros.append(f"Above gamma flip ${gamma_flip:.0f} — dealers stabilising ({gf_pts:+d}, dte-scaled)")
        elif is_bull and abs(gamma_flip - spot) / spot < near_t:
            if gf_penalty: score -= gf_penalty; cons.append(f"Near gamma flip ${gamma_flip:.0f} — vol risk ({-gf_penalty:+d}, dte-scaled)")
        elif is_bear and spot < gamma_flip:
            if gf_pts: score += gf_pts; pros.append(f"Below gamma flip ${gamma_flip:.0f} — trend amplification ({gf_pts:+d}, dte-scaled)")

    score = max(p["score_floor"], min(p["score_ceiling"], round(score)))

    gt = p["grade_thresholds"]
    if score >= gt["A"]:   grade, rec = "A", "OPEN"
    elif score >= gt["B"]: grade, rec = "B", "OPEN"
    elif score >= gt["C"]: grade, rec = "C", "OPEN_SMALL"
    elif score >= gt["D"]: grade, rec = "D", "OPEN_SMALL"
...
def _scan_one(symbol: str, dte_min: int, dte_max: int, min_earn_days: int, min_score: int,
    ...
    try:
        # 1. Earnings guard
        earn_info = _get_earn_info(symbol)
        earn_days = earn_info["earn_days"]
        earn_date = earn_info["earn_date"]
        ...
        for tt in candidate_types:
            eq = _entry_score(symbol, tt, spot, regime_bias, rs, iv_rank, pcr, put_wall, call_wall, gamma_flip,
                               ...
                               rsi_diff=rsi_diff, max_pain=max_pain, dte=dte, regime_detail=regime.get("regime", ""),
                               params=params)
        ...
        from ..services.scoring_params import get_params as _get_scoring_params
        _sp = params if params is not None else _get_scoring_params()
        if trade.get("width_capped_note"):
            trade.setdefault("cons", []).append(trade["width_capped_note"])
            trade["score"] = max(10, trade.get("score", 50) - _sp["width_capped_penalty"])
        ...
        if pop_val is not None and rr_val and rr_val > 0:
            ...
            if edge > _sp["pop_rr_strong_edge_pts"]:
                trade["score"] = min(100, trade.get("score", 50) + _sp["pop_rr_strong_edge_bonus"])
                trade.setdefault("pros", []).append(
                    f"POP {pop_val}% comfortably clears the {breakeven_pop:.0f}% breakeven for RR {rr_val} — real edge")
            elif edge > _sp["pop_rr_mild_edge_pts"]:
                trade["score"] = min(100, trade.get("score", 50) + _sp["pop_rr_mild_edge_bonus"])
                trade.setdefault("pros", []).append(
                    f"POP {pop_val}% above the {breakeven_pop:.0f}% breakeven for RR {rr_val}")
            elif edge < _sp["pop_rr_edge_threshold_pts"]:
                trade["score"] = max(10, trade.get("score", 50) + _sp["pop_rr_penalty"])
                ...
                trade["score"] = max(10, trade.get("score", 50) + _sp["pop_rr_thin_margin_penalty"])
                trade.setdefault("cons", []).append(
...
def api_scan():
    ...
    try:
        ...
        for fut, sym in bounded_as_completed(futs, timeout=90,
                on_timeout=lambda ks: print(f"[trade_opportunity_scanner] {len(ks)} symbol(s) "
                                             f"didn't finish in time, skipping: {ks[:20]}")):
            if fut is None:
        ...
        # wait=False is essential here: the whole point of bounded_as_completed
        # is to give up waiting after the timeout, but ThreadPoolExecutor's
        # own context-manager exit (or a bare .shutdown()) defaults to
        # wait=True and would block right back here until every submitted
        # future finishes -- silently undoing the timeout above.
...
def api_symbol_detail():
    ...
    for t in all_trades:
        eq = _entry_score(symbol, t["trade_type"], spot,
                          ...
                          rsi_diff=rsi_diff, max_pain=walls.get("max_pain"),
                          dte=dte, regime_detail=regime.get("regime", ""))
        scored.append({**t, **eq})
