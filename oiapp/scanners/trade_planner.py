...
def run_planner_scan(symbol):
    ...
    try:
        ...
        for fut, name in bounded_as_completed(futs, timeout=45,
                on_timeout=lambda ks: print(f"[trade_planner] {len(ks)} task(s) timed out: {ks}")):
            if fut is None:
                results[name] = {"status": "error", "detail": "timed out"}
                continue
