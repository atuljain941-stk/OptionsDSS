...
def _unique_dashboard_name(con, desired_name: str, exclude_id: Optional[int] = None) -> str:
    """Returns desired_name if free, otherwise desired_name with a numeric
    suffix that doesn't collide with any other dashboard."""
    base = desired_name
    candidate = base
    n = 2
    while True:
        row = con.execute(
            'SELECT id FROM scanner_dashboards WHERE lower(name)=lower(?)',
            (candidate,),
        ).fetchone()
        if row is None or (exclude_id is not None and int(row[0]) == int(exclude_id)):
            return candidate
        candidate = f'{base} ({n})'
        n += 1
...
def _save_dashboard_payload(payload: Dict[str, Any], dashboard_id: int | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # If a DIFFERENT dashboard already owns this exact name, don't crash —
        # silently de-duplicate the name (e.g. "My Scanner Dashboard (2)")
        # rather than letting sqlite's UNIQUE constraint raise a 500 that
        # leaves the tile/dashboard editor stuck open with no visible error.
        name = _unique_dashboard_name(con, name, exclude_id=existing_id)
        layout['settings']['name'] = name

        if existing_id is not None:
            con.execute(
                'UPDATE scanner_dashboards SET name=?, layout_json=?, updated_at=datetime(\'now\') WHERE id=?',
                (name, json.dumps(layout), existing_id),
            )
            did = existing_id
        else:
            con.execute(
