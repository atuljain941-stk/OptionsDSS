...
def _oi_rows(symbol, expiry):
    ...
    prev_date = dates[1]["date"] if len(dates) > 1 else None
    ...
    """, (symbol, expiry, latest_date)).fetchall()
    ...
    if prev_date:
        ...
        """, (symbol, expiry, prev_date)).fetchall()
        for r in prev_rows:
            try:
...
def _compute_ta(symbol):
    try:
        ...
        rets30=[math.log(C[i]/C[i-1]) for i in range(n-29,n+1) if C[i-1]>0]
        rets90=[math.log(C[i]/C[i-1]) for i in range(n-89,n+1) if C[i-1]>0] if n>=90 else rets30
...
def _walls(rows, spot, side=5, gex_info=None, sigma_1d=None, oi_change_filter=None):
    ...
    support_item = max(pb, key=lambda w: w["score"]) if pb else (max(puts, key=lambda w: w["score"]) if puts else None)
    resist_item  = max(ca, key=lambda w: w["score"]) if ca else (max(calls, key=lambda w: w["score"]) if calls else None)
...
def _score_gex_walls(rows, spot, gex_info=None, side=5):
    ...
    def _score_side(opt_type):
        ...
        out.sort(key=lambda x: (-x["score"], x["distance_pct"]))
        return out[:side]
...
def _weekly_relative_strength_profile(sym, benchmark="SPY", lookback=126):
    ...
    try:
        import yfinance as yf
        s = yf.Ticker(sym).history(period="1y", interval="1d")
        b = yf.Ticker(benchmark).history(period="1y", interval="1d")
...
def _fetch_tf_history(symbol, timeframe="1d"):
    ...
    try:
        if tf in ("2h", "4h"):
            df = yf.Ticker(sym).history(period="90d", interval="1h", prepost=False)
            return _resample_history(df, tf)
        if tf == "1h":
            return yf.Ticker(sym).history(period="60d", interval="1h", prepost=False)
        if tf in ("1w", "weekly"):
            return yf.Ticker(sym).history(period="2y", interval="1wk", prepost=False)
        return yf.Ticker(sym).history(period="1y", interval="1d", prepost=False)
    except Exception:
        return None
...
def _weekly_plan_composite(sym, spot, expiry, dte, profiles, walls, iv_rank, expected_move, pcr, vix=None):
...
def api_weekly():
    ...
    for _p in ["1y","6mo","3mo"]:
        try:
            h = yf.Ticker(sym).history(period=_p, interval="1d")
    ...
    plan_score = _weekly_plan_composite(sym, spot, exp, dte, profiles, walls, iv_rank, expected_move, pcr, vix=vix_ctx)
...
def _build_trade_plan(spot, gex_info, walls, ta, score, sigma_1d):
    ...
    top_put_sig = (sig.get("top_put_walls") or [{}])[0]
    top_call_sig = (sig.get("top_call_walls") or [{}])[0]
...
def compute_gex_reliability(sym: str, spot: float, gex_total: float,
    ...
    try:
        ...
        _vx = _yf.Ticker("^VIX").history(period="5d")
