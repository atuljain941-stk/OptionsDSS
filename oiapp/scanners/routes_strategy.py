...
def _compute_ta(symbol):
    try:
        ...
        rets90 = [math.log(C[i]/C[i-1]) for i in range(max(1,n-89),n+1) if C[i-1]>0]
        rv30 = rv(rets30); rv90 = rv(rets90)
        iv_rank = min(100, max(0, round((rv30/rv90)*50))) if rv90 else 40
...
def analyze_strategy():
    ...
    if not sym or sym == "ALL":
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
            results = list(ex.map(_one, syms[:30]))
