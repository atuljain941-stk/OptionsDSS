...
def _build_option_position(
    ...
    if trade_type == "CS":
        ...
        # Genuinely respects strike_mode now -- previously this branch
        # ignored strike_mode/short_delta entirely and always used a
        # fixed OTM-percentage band, regardless of what was requested.
        # target_delta was computed but only ever used for AFTER-the-fact
        # reporting (actual_short_call_delta/actual_short_put_delta
        # below), never to actually place a strike -- meaning running
        # this with strike_mode="delta" silently produced OTM-band
        # strikes instead, with no error or warning. Fixed by actually
        # branching on strike_mode, and added "expected_move" (1-sigma)
        # as a genuine third mode using the standard S*(1 +/- sigma*sqrt(T))
        # formula, reusing the same hv value already computed above --
        # not a separate estimate.
        if strike_mode == "delta":
            short_put = round(_strike_from_delta(S, dte, hv, "PUT", target_delta), 2)
            short_call = round(_strike_from_delta(S, dte, hv, "CALL", target_delta), 2)
        elif strike_mode == "expected_move":
            T = max(float(dte), 1.0) / 365.0
            move = S * max(0.01, hv) * math.sqrt(T)
            short_put = round(max(0.01, S - move), 2)
            short_call = round(S + move, 2)
        else:  # "otm" -- original fixed-band behavior, unchanged for anyone already relying on it
...
def _run_chronological_backtest(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if strike_mode not in {"delta", "otm", "expected_move"}:
        strike_mode = "delta"
    ...
    try:
        ...
        # backtest window fails with "not enough future bars to reach
        # selected DTE" -- not a data gap, just never having fetched
        # far enough forward in the first place. This is especially
        # visible when run_for_days is close to dte (e.g. both 30),
        # since then essentially every trade in the whole window falls
        # into that gap. +15 calendar days of slack on top of dte for
        # weekends/holidays between the DTE target and the next actual
        # trading bar (_expiry_index rounds forward to the next bar).
        fetch_end = end_date + timedelta(days=int(dte) + 15)
        ...
        for fut, sym in bounded_as_completed(futures, timeout=120,
                on_timeout=lambda ks: print(f"[backtest] {len(ks)} symbol(s) timed out: {ks[:20]}")):
            if fut is None:
                histories[sym] = SymbolHistory(sym, pd.DataFrame(), {}, "timed out")
    ...
    # V112 fix: "Run for (days)" should control how many days are
    # scanned for NEW entries, not truncate the whole simulation --
    # a trade opened near the end of that window still needs to walk
    # forward its full DTE to know whether it closed a winner or
    # loser. Previously the day-walk itself stopped at end_date, so
    # any trade opened within `dte` days of the window's end could
    # never be checked for closing at all (this is what produced
    # "still open" results whenever run_for_days was close to or
    # shorter than dte). Two separate ranges now: `entry_days` (the
    # user's actual N-day scan window, used only to decide which days
    # are eligible to open a NEW position) and `all_days` (extended
    # past end_date by dte + buffer, used for the closing check on
    # every day so already-open positions get their real chance to
    # resolve). New entries are never opened past end_date; closing
    # checks always run for every open position on every walked day.
    closing_walk_end = end_date + timedelta(days=int(dte) + 15)
    entry_days = {d for sym in valid_symbols for d in _trading_dates(histories[sym].daily) if start_date <= d <= end_date}
    all_days = sorted({d for sym in valid_symbols for d in _trading_dates(histories[sym].daily) if start_date <= d <= closing_walk_end})
    if not all_days:
    ...
    for day in all_days:
        ...
        # New-entry scanning only happens within the user's actual
        # run-for-days window -- days beyond end_date (the closing-walk
        # extension) only process the exit check above, never open a
        # new position.
        ...
        for ctx in contexts:
            ...
            if pos is None:
                ...
                continue
            # Same diagnostic capture as the forward-return backtest --
            # RSI/trend/momentum context already computed by ctx purely
            # to evaluate the query, attached here so it survives
            # through _close_position (which does `out = dict(position)`,
            # preserving any extra keys) into the final closed-trade
            # record. Lets winners-vs-losers diagnostics work on your
            # ACTUAL options-strategy P&L results, not just raw price
            # return.
            rsi14_v = ctx.get("rsi14")
            close_v, ema20_v, ema50_v = ctx.get("close"), ctx.get("ema20"), ctx.get("ema50")
            trend_regime = None
            if all(v is not None for v in (close_v, ema20_v, ema50_v)):
                if close_v > ema20_v > ema50_v:
                    ...
                    trend_regime = "mixed"
            entry_idx_for_bar = entry_indices.get(sym, 0)
            bar_is_up = None
            try:
                bar_is_up = bool(histories[sym].daily["Close"].iloc[entry_idx_for_bar] > histories[sym].daily["Open"].iloc[entry_idx_for_bar])
            except Exception:
                bar_is_up = None
            counter_trend = None
            if trend_regime and bar_is_up is not None:
                counter_trend = (bar_is_up and trend_regime == "downtrend") or (not bar_is_up and trend_regime == "uptrend")
            pos["entry_diagnostics"] = {
                "rsi14": round(rsi14_v, 1) if rsi14_v is not None else None,
                "overbought": (rsi14_v is not None and rsi14_v > 70),
                "oversold": (rsi14_v is not None and rsi14_v < 30),
                "trend_regime": trend_regime,
                "counter_trend": counter_trend,
                "macd_hist": round(ctx.get("macd_hist"), 4) if ctx.get("macd_hist") is not None else None,
                "rsi_diff_90": round(ctx.get("rsi_diff_90"), 1) if ctx.get("rsi_diff_90") is not None else None,
            }
    ...
    stats = _stats(closed_trades, open_public, skipped, len(entry_days))
    ...
    result = {
        ...
        "stats": stats,
        "diagnostic_comparison": _build_diagnostic_comparison_from_trades(closed_trades),
        "recommendations": _generate_recommendations(_build_diagnostic_comparison_from_trades(closed_trades)),
...
@backtest_bp.route("/api/forward_return", methods=["POST"])
def api_backtest_forward_return():
    """Plain underlying %-return backtest: how much did the price move
    N days after the query fired, no options simulated. Separate from
    /api/run (which simulates a real options position -- strike, delta,
    DTE, exit management) because "what return did this signal generate"
    and "what would a credit spread on this signal have made" are
    different questions, and running the former through the options
    engine would mix in theta/IV effects that have nothing to do with
    the underlying's own move."""
    _ensure_scanner_tables()
    payload = request.get_json(force=True) or {}
    try:
        result = _run_forward_return_backtest(payload)
        return jsonify(result)
    except ValueError as e:
        return jsonify({"ok": False, "error": str(e)}), 400
    except Exception as e:
        return jsonify({"ok": False, "error": f"Forward-return backtest failed: {e}"}), 500


def _run_forward_return_backtest(config: Dict[str, Any]) -> Dict[str, Any]:
    start_date = _parse_date(config.get("start_date") or config.get("start"))
    end_date = _parse_date(config.get("end_date"), date.today())
    if start_date >= end_date:
        raise ValueError("Start date must be before the end of available data")

    forward_days = _parse_nonnegative_int(config.get("forward_days"), 30, 252)
    if forward_days <= 0:
        forward_days = 30
    ...
    earn_raw = config.get("earnings_avoid_days") if "earnings_avoid_days" in config else config.get("earnings_avoid")
    earnings_avoid_days = _parse_nonnegative_int(earn_raw, 0, 365)
    ...
    # Fetch a buffer of extra calendar days past end_date so signals near
    # the end of the window can still have their forward return measured
    # (roughly forward_days x 1.6 calendar days per trading day, generous
    # for weekends/holidays).
    fetch_end = end_date + timedelta(days=int(forward_days * 1.6) + 10)
    ...
    try:
        ...
        from ..services.bounded_wait import bounded_as_completed
        for fut, sym in bounded_as_completed(futures, timeout=120,
                on_timeout=lambda ks: print(f"[backtest forward_return] {len(ks)} symbol(s) timed out: {ks[:20]}")):
            if fut is None:
                histories[sym] = SymbolHistory(sym, pd.DataFrame(), {}, "timed out")
                errors.append({"symbol": sym, "error": "timed out"})
                continue
    ...
    earnings_cache: Dict[str, Dict[str, Any]] = {}
    if get_symbol_calendar is not None and earnings_avoid_days > 0:
        for s in valid_symbols:
            try:
                earnings_cache[s] = get_symbol_calendar(s) or {}
            except Exception:
    ...
    for day in all_days:
        ...
        for ctx in contexts:
            ...
            hist = histories[sym]
            entry_idx = entry_indices[sym]
            entry_price = float(hist.daily["Close"].iloc[entry_idx])
            forward_idx = entry_idx + forward_days
            if forward_idx >= len(hist.daily):
                skipped.append({"date": day.isoformat(), "symbol": sym, "reason": f"not enough future data yet ({forward_days}d forward)"})
                continue
            forward_price = float(hist.daily["Close"].iloc[forward_idx])
            pct_return = (forward_price / entry_price - 1.0) * 100.0

            bench_return = None
            if benchmark_hist is not None and not benchmark_hist.daily.empty:
                b_entry_idx = _price_row_for_day(benchmark_hist.daily, day)
                if b_entry_idx is not None:
                    b_forward_idx = b_entry_idx + forward_days
                    if b_forward_idx < len(benchmark_hist.daily):
                        b_entry_price = float(benchmark_hist.daily["Close"].iloc[b_entry_idx])
                        b_forward_price = float(benchmark_hist.daily["Close"].iloc[b_forward_idx])
                        bench_return = (b_forward_price / b_entry_price - 1.0) * 100.0

            forward_date_val = hist.daily.index[forward_idx]

            # Diagnostic context AT ENTRY -- this was already computed
            # by _build_ctx_for_day purely to evaluate the query above;
            # capturing it here (rather than discarding it) answers
            # "why did winners differ from losers" directly: was RSI
            # overbought/oversold, what was the prevailing trend regime
            # relative to this signal's direction, momentum (MACD hist)
            # direction. See the "diagnostic_comparison" block below the
            # signals list for the actual winners-vs-losers breakdown.
            rsi14_v = ctx.get("rsi14")
            close_v, ema20_v, ema50_v, ema200_v = ctx.get("close"), ctx.get("ema20"), ctx.get("ema50"), ctx.get("ema200")
            trend_regime = None
            if all(v is not None for v in (close_v, ema20_v, ema50_v)):
                if close_v > ema20_v > ema50_v:
                    trend_regime = "uptrend"
                    ...
                    trend_regime = "mixed"
            # "Counter-trend" = this bar closed UP (a bull-type signal)
            # while the broader trend was down, or closed DOWN (a
            # bear-type signal) while the broader trend was up -- the
            # "was it just a false/shake candle against the real trend"
            # question, made explicit instead of needing to eyeball it.
            bar_is_up = None
            if entry_idx > 0:
                try:
                    bar_is_up = bool(hist.daily["Close"].iloc[entry_idx] > hist.daily["Open"].iloc[entry_idx])
                except Exception:
                    bar_is_up = None
            counter_trend = None
            if trend_regime and bar_is_up is not None:
            ...
            signals.append({
                ...
                "forward_date": forward_date_val.date().isoformat() if hasattr(forward_date_val, "date") else str(forward_date_val),
                "forward_price": _round_money(forward_price),
                "return_pct": round(pct_return, 2),
                "benchmark_return_pct": round(bench_return, 2) if bench_return is not None else None,
                "excess_return_pct": round(pct_return - bench_return, 2) if bench_return is not None else None,
                "entry_diagnostics": {
                    "rsi14": round(rsi14_v, 1) if rsi14_v is not None else None,
                    "overbought": (rsi14_v is not None and rsi14_v > 70),
                    "oversold": (rsi14_v is not None and rsi14_v < 30),
    ...
    if not signals:
        return {
            "ok": True, "signal_count": 0, "signals": [], "skipped": skipped[:200],
            "summary": None, "errors": errors,
        }
    ...
    summary = {
        ...
        "win_rate_pct": round(len(wins) / len(returns) * 100.0, 1),
        "avg_return_pct": round(sum(returns) / len(returns), 2),
        "median_return_pct": round(sorted_returns[len(sorted_returns) // 2], 2),
        "best_return_pct": round(max(returns), 2),
        "worst_return_pct": round(min(returns), 2),
        "forward_days": forward_days,
    }
    bench_returns = [s["benchmark_return_pct"] for s in signals if s["benchmark_return_pct"] is not None]
    if bench_returns:
        summary["avg_benchmark_return_pct"] = round(sum(bench_returns) / len(bench_returns), 2)
        excess = [s["excess_return_pct"] for s in signals if s["excess_return_pct"] is not None]
        summary["avg_excess_return_pct"] = round(sum(excess) / len(excess), 2) if excess else None

    diagnostic_comparison = _build_diagnostic_comparison(signals)
    recommendations = _generate_recommendations(diagnostic_comparison)

    return {
        "ok": True, "signal_count": len(signals), "signals": signals,
        "summary": summary, "diagnostic_comparison": diagnostic_comparison,
        "recommendations": recommendations,
        "skipped": skipped[:200], "errors": errors,
    }


def _build_diagnostic_comparison(signals: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Winners (return_pct > 0) vs losers (<= 0), compared across the
    entry_diagnostics captured per signal -- this is the actual
    "why did winners differ from losers" answer: if losers were
    overbought/counter-trend far more often than winners, that's a
    concrete, checkable filter to add to the query (e.g. AND NOT
    RSI14 > 70, or AND trend confirmation) rather than a guess.
    """
    ...
    def _avg(rows: List[Dict[str, Any]], field: str) -> Optional[float]:
        vals = [r["entry_diagnostics"].get(field) for r in rows if r.get("entry_diagnostics", {}).get(field) is not None]
        return round(sum(vals) / len(vals), 2) if vals else None

    def _pct_true(rows: List[Dict[str, Any]], field: str) -> Optional[float]:
        vals = [r["entry_diagnostics"].get(field) for r in rows if r.get("entry_diagnostics", {}).get(field) is not None]
        return round(sum(1 for v in vals if v) / len(vals) * 100.0, 1) if vals else None

    def _regime_distribution(rows: List[Dict[str, Any]]) -> Dict[str, float]:
        regimes = [r["entry_diagnostics"].get("trend_regime") for r in rows if r.get("entry_diagnostics", {}).get("trend_regime")]
        if not regimes:
            return {}
        out = {}
        for regime in ("uptrend", "downtrend", "mixed"):
            count = sum(1 for r in regimes if r == regime)
            out[regime] = round(count / len(regimes) * 100.0, 1)
        return out

    return {
        "winner_count": len(winners), "loser_count": len(losers),
        "avg_rsi14": {"winners": _avg(winners, "rsi14"), "losers": _avg(losers, "rsi14")},
        "pct_overbought_at_entry": {"winners": _pct_true(winners, "overbought"), "losers": _pct_true(losers, "overbought")},
        "pct_oversold_at_entry": {"winners": _pct_true(winners, "oversold"), "losers": _pct_true(losers, "oversold")},
        "pct_counter_trend_at_entry": {"winners": _pct_true(winners, "counter_trend"), "losers": _pct_true(losers, "counter_trend")},
        "trend_regime_distribution": {"winners": _regime_distribution(winners), "losers": _regime_distribution(losers)},
        "avg_macd_hist": {"winners": _avg(winners, "macd_hist"), "losers": _avg(losers, "macd_hist")},
        "avg_rsi_diff_90": {"winners": _avg(winners, "rsi_diff_90"), "losers": _avg(losers, "rsi_diff_90")},
    }


def _build_diagnostic_comparison_from_trades(closed_trades: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Same winners-vs-losers breakdown as _build_diagnostic_comparison,
    adapted for the options-strategy chronological backtest's closed
    trade shape -- uses the `winner` boolean already on each closed
    trade (from _close_position / _close_position_early) instead of a
    plain return_pct, since here "winner" reflects the actual options
    P&L outcome (credit spread landed OTM, etc.), not just whether the
    underlying went up."""
    winners = [t for t in closed_trades if t.get("winner")]
    losers = [t for t in closed_trades if not t.get("winner")]

    def _avg(rows: List[Dict[str, Any]], field: str) -> Optional[float]:
        vals = [r.get("entry_diagnostics", {}).get(field) for r in rows if r.get("entry_diagnostics", {}).get(field) is not None]
        return round(sum(vals) / len(vals), 2) if vals else None
    ...
    def _regime_distribution(rows: List[Dict[str, Any]]) -> Dict[str, float]:
        regimes = [r.get("entry_diagnostics", {}).get("trend_regime") for r in rows if r.get("entry_diagnostics", {}).get("trend_regime")]
        if not regimes:
            return {}
        out = {}
        for regime in ("uptrend", "downtrend", "mixed"):
...
def _generate_recommendations(diagnostic_comparison: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Turns the winners-vs-losers diagnostic comparison into ranked,
    plain-English recommendations -- sorted by the SIZE of the actual
    gap (biggest separation = most likely to be a useful filter), and
    explicit about DIRECTION. This matters because a naive "avoid X"
    assumption can be backwards -- e.g. if winners were counter-trend
    MORE often than losers in the actual data, recommending "avoid
    counter-trend" would make things worse, not better. Only suggests
    pasteable query syntax where the underlying function is confirmed
    to exist in FUNCTION_CATALOG (ema(expr, period) is real; there is
    NO raw RSI-level function in this DSL, only RSIDiff/RSIDiff90
    which measure RSI relative to its own long-term average, not
    absolute overbought/oversold -- so RSI-level findings get a plain-
    English note instead of fabricated syntax).
    """
    recs: List[Dict[str, Any]] = []
    dc = diagnostic_comparison or {}
    if not dc:
        return recs

    def _gap(metric: Dict[str, Any]) -> Optional[float]:
        w, l = metric.get("winners"), metric.get("losers")
        if w is None or l is None:
            return None
        return round(l - w, 2)  # positive = losers higher

    trend_snippet = "(close > ema(close,20) and ema(close,20) > ema(close,50)) or (close < ema(close,20) and ema(close,20) < ema(close,50))"

    # Overbought -- no raw-RSI-level primitive exists, so this stays a
    # plain-English insight rather than fabricated pasteable syntax.
    gap = _gap(dc.get("pct_overbought_at_entry", {}))
    if gap is not None and abs(gap) >= 8:
        if gap > 0:
            recs.append({
                "metric": "Overbought at entry (RSI14 > 70)", "gap": gap, "direction": "losers_higher",
                "insight": f"Losers were overbought {gap} points more often than winners at entry.",
                "suggestion": "This query language has no raw RSI-level function (only RSIDiff/RSIDiff90, which measure RSI vs its own long-term average, not absolute overbought/oversold) -- no exact pasteable equivalent to 'RSI < 70' exists here. Worth tracking manually before assuming it's addable as a filter.",
                "suggested_query_snippet": None,
            })
        else:
            recs.append({
                "metric": "Overbought at entry (RSI14 > 70)", "gap": gap, "direction": "winners_higher",
                "insight": f"Winners were actually overbought {abs(gap)} points MORE often than losers -- the opposite of the usual assumption. Do not add an 'avoid overbought' filter based on this data.",
                "suggestion": None, "suggested_query_snippet": None,
            })

    # Counter-trend
    gap = _gap(dc.get("pct_counter_trend_at_entry", {}))
    if gap is not None and abs(gap) >= 5:
        if gap > 0:
            recs.append({
                "metric": "Counter-trend entry", "gap": gap, "direction": "losers_higher",
                "insight": f"Losers were counter-trend {gap} points more often than winners.",
                "suggestion": "Consider requiring the candle direction to agree with the prevailing EMA20/50 trend.",
                "suggested_query_snippet": trend_snippet,
            })
        else:
            recs.append({
                "metric": "Counter-trend entry", "gap": gap, "direction": "winners_higher",
                "insight": f"Winners were counter-trend {abs(gap)} points MORE often than losers -- the opposite of the intuitive assumption. Adding an 'avoid counter-trend' filter would likely hurt performance here, not help. This may mean the signal works better as a reversal/fade play than a trend-continuation one.",
                "suggestion": None, "suggested_query_snippet": None,
            })

    # "Mixed" (choppy, no clean EMA alignment) trend regime -- directly
    # answers the original "was it just a shake/false candle" question.
    tr = dc.get("trend_regime_distribution", {}) or {}
    w_mixed = (tr.get("winners") or {}).get("mixed", 0)
    l_mixed = (tr.get("losers") or {}).get("mixed", 0)
    mixed_gap = round(l_mixed - w_mixed, 2)
    if abs(mixed_gap) >= 8:
        if mixed_gap > 0:
            recs.append({
                "metric": "Choppy/'mixed' trend regime at entry", "gap": mixed_gap, "direction": "losers_higher",
                "insight": f"Losers entered during a choppy/mixed regime (no clean EMA20/50 alignment) {mixed_gap} points more often than winners ({l_mixed}% vs {w_mixed}%).",
                "suggestion": "Require a clean trend (EMA20 and EMA50 aligned in one direction) at entry, rejecting choppy/sideways conditions -- this is the concrete, checkable version of 'was it just a shake candle'.",
                "suggested_query_snippet": trend_snippet,
            })
        else:
            recs.append({
                "metric": "Choppy/'mixed' trend regime at entry", "gap": mixed_gap, "direction": "winners_higher",
                "insight": f"Winners entered during a choppy/mixed regime MORE often than losers ({w_mixed}% vs {l_mixed}%) -- do not add a 'require clean trend' filter based on this data.",
                "suggestion": None, "suggested_query_snippet": None,
            })

    # MACD histogram -- flagged with a scale caveat since raw MACD
    # isn't normalized across symbols/price levels, making a
    # cross-sectional average comparison inherently noisier.
    gap = _gap(dc.get("avg_macd_hist", {}))
    if gap is not None and abs(gap) > 0.1:
        higher = "Losers" if gap > 0 else "Winners"
        recs.append({
            "metric": "MACD histogram magnitude", "gap": None, "direction": "losers_higher" if gap > 0 else "winners_higher",
            "insight": f"{higher} had a {'higher' if gap>0 else 'lower'} average MACD histogram at entry, but raw MACD isn't normalized across symbols/price levels -- this comparison mixes different scales. Treat as a weak signal, not a clear filter.",
            "suggestion": None, "suggested_query_snippet": None,
        })

    recs.sort(key=lambda r: abs(r.get("gap") or 0), reverse=True)
    return recs
...
# =====================================================================
# V136: OI-wall / OI-buildup backtest against the real daily options
# chain archive (the `options` table in db.py -- genuinely append-
# only, unlike the rolling caches elsewhere in the app). See
# oi_wall_backtest.py for the hypothesis definitions and caveats.
# =====================================================================

@backtest_bp.route("/api/oi_explore", methods=["GET"])
def api_oi_explore():
    from .oi_wall_backtest import explore_oi_history
    try:
        return jsonify(explore_oi_history())
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@backtest_bp.route("/api/oi_wall_backtest", methods=["GET"])
def api_oi_wall_backtest():
    from .oi_wall_backtest import backtest_oi_walls
    raw_symbols = request.args.get("symbols", "").strip()
    watchlist_id = request.args.get("watchlist_id", "").strip()
    ...
    if not symbols:
        return jsonify({"ok": False, "error": "no symbols found -- pass ?symbols=A,B,C or ?watchlist_id=N, or select a default watchlist"}), 400
    try:
        max_symbols = int(request.args.get("max_symbols") or 0)
        if max_symbols > 0:
            symbols = symbols[:max_symbols]
        dte_days = int(request.args.get("dte_days", "5"))
        num_runs = int(request.args.get("num_runs", "3"))
        start_date = request.args.get("start_date", "").strip()
        min_strength = float(request.args.get("min_wall_strength_pct", "15"))
        min_oi = int(request.args.get("min_total_oi", "1000"))
        max_wall_distance_pct_raw = request.args.get("max_wall_distance_pct", "").strip()
        max_wall_distance_pct = float(max_wall_distance_pct_raw) if max_wall_distance_pct_raw else None
        use_delta_filter = request.args.get("use_delta_filter", "false").strip().lower() == "true"
        delta_min = float(request.args.get("delta_min", "0.10"))
        delta_max = float(request.args.get("delta_max", "0.40"))
        return jsonify(backtest_oi_walls(
            symbols, dte_days=dte_days, start_date=start_date, num_runs=num_runs,
            min_wall_strength_pct=min_strength, min_total_oi=min_oi,
            max_wall_distance_pct=max_wall_distance_pct,
            use_delta_filter=use_delta_filter, delta_min=delta_min, delta_max=delta_max,
        ))
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
