...
def _futures_active_contracts(symbol=None):
    ...
    try:
        from ..services.futures_oi_schwab import SCHWAB_ROOTS, _get_quarterly_contracts, normalize_futures_symbol
        if symbol:
        ...
        for s in syms:
            ...
            if not root:
                continue
            for c in _get_quarterly_contracts(root, 6):
...
def api_oi_change():
    ...
    try:
        ...
        immediate_prev_date = dates[1] if len(dates) > 1 else None
        for candidate_date in dates[1:]:
        ...
        comparison_note = ""
        comparison_mode = "previous_distinct_snapshot" if skipped_identical_dates else "previous_snapshot"
        ...
        try:
            ...
            for s in syms:
                ...
                if row.get("error") and stored <= 0:
                    ...
                    errors.append(f"{s}: no usable OI rows stored from Schwab/CME")
                ...
                _futures_job_update(
                    ...
                    message=f"Fetched {len(results)}/{len(syms)} symbols using Schwab → CME → COT...",
                )
        except Exception as e:
