...
def _setting_float(key: str, default: float) -> float:
    try:
        return float(_app_setting_get(key, str(default)) or default)
    except (TypeError, ValueError):
        return default


def _setting_set_float(key: str, value: float) -> None:
    _app_setting_set(key, str(float(value)))


def _deep_loss_alerts_enabled() -> bool:
    return _setting_bool("journal_deep_loss_alerts_enabled", True)


def _deep_loss_alert_threshold_pct() -> float:
    # Global, applies to every open position -- 75% by default: a 50%
    # loss on an option position is common and often still recoverable,
    # but a 75%+ loss is much rarer and, per the reasoning behind this
    # alert, close to a point of no return in practice even when the
    # position isn't technically at max loss yet. Configurable per-user
    # via the Signal Notifier page -- reads the SAME setting key
    # signal_notifier.set_config() writes to (signal_notifier_journal_
    # deep_loss_threshold_pct), not a separate one, so there's exactly
    # one number in effect regardless of which surface last changed it.
    return _setting_float("signal_notifier_journal_deep_loss_threshold_pct", 75.0)
...
def _live_option_price(symbol, expiry, strike, side):
    ...
    # Tastytrade first, when connected -- real broker bid/ask/mid instead
    # of yfinance's options data, which is exactly what was causing P&L
    # to show blank: fetch_chain() below returning nothing for a given
    # strike/expiry (or timing out, or hitting yfinance's own known NaN
    # parser issue) means unrealised_pnl never gets set at all -- not a
    # wrong number, no number. Falls through unchanged to the existing
    # chain below if Tastytrade isn't connected or this specific contract
    # can't be quoted.
    try:
        from ..scanners.realtime_dashboard import get_option_quote
        tt = get_option_quote(symbol, expiry, k, opt_type)
        if tt and not tt.get("error") and tt.get("mid") is not None:
            return round(float(tt["mid"]), 4)
    except Exception:
        pass
...
def _analyze_position_oi_shift(t, live, min_pct_change=25.0, min_abs_change=500):
    """Check whether OI at THIS position's own short strikes has shifted
    materially since it was opened, and say what that implies.

    Why this is worth its own check rather than folding into the price-
    based rules above: price and OI can disagree, and when they do, the
    OI side often moves first. A short strike that's still comfortably
    OTM on price but is seeing large fresh OI build is the market
    positioning around a level the position depends on staying clear of
    -- that's a reason to pay attention BEFORE price gets there, which
    is the whole point of managing early rather than at the breach.

    Deliberately reports rather than auto-decides: OI build at a short
    strike is genuinely ambiguous on its own (it can be other sellers
    piling into the same view, which supports the level holding, or
    buyers positioning for a move through it). So this surfaces the
    shift with its direction and size and pairs it with the position's
    existing price-based state -- it does NOT override the action from
    _compute_live_pnl, which is grounded in P&L and DTE facts that are
    unambiguous in a way raw OI change is not.

    Thresholds: both a percentage AND an absolute floor must be met, so
    a strike going from 4 to 8 contracts (+100%) doesn't register as
    "significant" while a genuinely liquid strike moving 500+ does.
    """
    try:
        from ..scanners.spy_strategies import _oi_rows
    except Exception:
        return None

    tt = (_row_value(t, "trade_type", "") or "").upper()
    if tt not in ("PS", "CS", "IC"):
        return None  # credit-spread structures only -- these are the ones with short strikes to defend
    symbol = (_row_value(t, "symbol", "") or "").upper()
    expiry = _row_value(t, "expiry", "") or ""
    ...
    watch = []
    if tt == "IC":
        if resolved.get("put_sell") is not None:
            watch.append(("put", _safe(resolved.get("put_sell"))))
        if resolved.get("call_sell") is not None:
            watch.append(("call", _safe(resolved.get("call_sell"))))
    elif tt == "PS":
        watch.append(("put", _safe(resolved.get("sell") or _row_value(t, "short_strike", None))))
    elif tt == "CS":
        watch.append(("call", _safe(resolved.get("sell") or _row_value(t, "short_strike", None))))
    watch = [(side, k) for side, k in watch if k is not None]
    if not watch:
        return None
    ...
    for r in rows:
        try:
            by_key[(str(r.get("type") or "").lower(), float(r.get("strike") or 0))] = r
        except Exception:
            continue

    findings = []
    for side, strike in watch:
        row = by_key.get((side, float(strike)))
        if not row:
            continue
        oi_now = int(row.get("oi") or 0)
        oi_chg = int(row.get("oi_change") or 0)
        if oi_now <= 0 or oi_chg == 0:
            continue
        prior = oi_now - oi_chg
        pct = (oi_chg / prior * 100.0) if prior > 0 else 0.0
        if abs(oi_chg) < min_abs_change or abs(pct) < min_pct_change:
            continue
        findings.append({
            "side": side, "strike": strike, "oi": oi_now,
            "oi_change": oi_chg, "oi_change_pct": round(pct, 1),
    ...
    for f in findings:
        dist_note = ""
        if spot:
            dist_pct = abs(f["strike"] - spot) / spot * 100
            dist_note = f", {dist_pct:.1f}% from spot ${spot}"
        if f["direction"] == "building":
            notes.append(
                f"Short {f['side']} {f['strike']}: OI {f['oi_change']:+,} ({f['oi_change_pct']:+.0f}%) to {f['oi']:,}{dist_note}. "
                f"Fresh positioning is concentrating at a strike this trade needs to stay clear of -- worth watching, "
                f"though build alone is ambiguous (could be other sellers agreeing with the level, or buyers positioning through it)."
            )
        else:
            notes.append(
                f"Short {f['side']} {f['strike']}: OI {f['oi_change']:+,} ({f['oi_change_pct']:+.0f}%) to {f['oi']:,}{dist_note}. "
                f"Positioning is unwinding at this strike -- the level this trade leans on is getting thinner."
            )
...
def _maybe_log_trade_pnr_alert(t, live):
    try:
        ...
        if breached and not last_breached:
            # Carry the ACTUAL computed action and its reasoning, not just
            # "PNR breached". _compute_live_pnl already works out
            # ROLL-vs-CLOSE (including which side to roll and roughly
            # where), the profit/loss rules, and DTE/gamma risk -- all of
            # that was being discarded here in favour of generic status
            # text, leaving the alert to say something is wrong without
            # saying what to do about it, which is the whole point of
            # getting the alert in time.
            action = live.get("action") or "Review position"
            action_reason = live.get("action_reason") or live.get("pnr_status") or "PNR breached"
            urgency = live.get("urgency") or "high"
            pct = live.get("pct_of_max_profit")
            pnl = live.get("unrealised_pnl")
            facts = []
            if live.get("spot") is not None:
                facts.append(f"spot ${live.get('spot')}")
            if live.get("dte") is not None:
                facts.append(f"{live.get('dte')} DTE")
            if pnl is not None:
                facts.append(f"P&L ${pnl:.0f}")
            if pct is not None:
                facts.append(f"{pct:.0f}% of max")
            fact_line = " · ".join(facts)
            # OI-shift context, when there is any -- see
            # _analyze_position_oi_shift's docstring for why this is
            # reported alongside the price-based action rather than
            # allowed to change it.
            oi_shift = _analyze_position_oi_shift(t, live)
            detail = f"{action_reason}\n{fact_line}" if fact_line else action_reason
            if oi_shift and oi_shift.get("notes"):
                detail += "\n\nOI shift at your short strikes:\n" + "\n".join(oi_shift["notes"])
            log_alert_notification(
                "PNR_BREACH",
                f"{t.get('symbol','').upper()} — {action}",
                detail,
                symbol=(t.get("symbol") or "").upper(),
                trade_id=t.get("id"),
                severity="error" if urgency in ("critical", "high") else "warn",
                source="journal",
                metadata={"trade_id": t.get("id"), "symbol": t.get("symbol"), "pnr": live.get("pnr"), "pnr_upper": live.get("pnr_upper"), "spot": live.get("spot"), "trade_type": t.get("trade_type"), "action": action, "urgency": urgency, "oi_shift": (oi_shift or {}).get("findings")},
            )
...
def _maybe_log_trade_sl_tp_alert(t, live):
    """Same pattern as _maybe_log_trade_pnr_alert, for the user-set
    Stop Loss Price / Take Profit Price Target fields -- compares the
    trade's live current_mark (option points, same value shown as
    "Mark" in the UI) against whichever threshold(s) were set, only
    fires once per breach (tracked via sl_tp_alert_last_state so it
    doesn't re-fire every scan), and uses the exact alert text
    requested: "Stop loss price triggered" / "Take profit, price
    target achieved"."""
    try:
        t = _as_dict(t)
        ...
        if sl is None and tp is None:
            return
        mark = live.get("current_mark")
        if mark is None:
            return
        last_state = t.get("sl_tp_alert_last_state") or "none"

        # Direction convention: a credit strategy's mark should FALL
        # toward 0 for profit and RISE for loss (you bought it back
        # cheaper than you sold it); a debit strategy's mark should
        # RISE for profit. Use the trade's own net_premium sign (same
        # value already shown as "Net Premium" in the UI) to pick the
        # right direction instead of assuming one.
        is_credit = (t.get("net_premium") or 0) >= 0
        tp_hit = tp is not None and ((mark <= tp) if is_credit else (mark >= tp))
        sl_hit = sl is not None and ((mark >= sl) if is_credit else (mark <= sl))

        new_state = "tp_hit" if tp_hit else ("sl_hit" if sl_hit else "none")
        if new_state != "none" and new_state != last_state:
            if new_state == "tp_hit":
                title = f"{t.get('symbol','').upper()} take profit, price target achieved"
                detail = f"Mark {mark} vs target {tp}"
                severity = "success"
            else:
                title = f"{t.get('symbol','').upper()} stop loss price triggered"
                detail = f"Mark {mark} vs stop {sl}"
                severity = "warn"
            log_alert_notification(
                "SL_TP_HIT", title, detail,
                symbol=(t.get("symbol") or "").upper(), trade_id=t.get("id"),
                severity=severity, source="journal",
                metadata={"trade_id": t.get("id"), "symbol": t.get("symbol"), "mark": mark,
                          "stop_loss_price": sl, "target_price": tp, "state": new_state},
            )
        if new_state != last_state:
            con = _conn()
            try:
                con.execute(
                    "UPDATE trades SET sl_tp_alert_last_state=?, sl_tp_alert_last_sent_at=? WHERE id=?",
                    (new_state, datetime.now().isoformat(timespec='seconds') if new_state != "none" else t.get("sl_tp_alert_last_sent_at"), t.get("id")),
                )
                con.commit()
            finally:
                con.close()
...
def _maybe_log_deep_loss_alert(t, live):
    """Same shape as _maybe_log_trade_pnr_alert: fires once when a
    position first crosses the global deep-loss threshold (not on every
    check while it stays past it), using the trade's own
    deep_loss_alert_last_breached column to avoid repeat alerts."""
    try:
        t = _as_dict(t)
        if not _is_open_option_trade(t) or not _deep_loss_alerts_enabled():
            return
        threshold = _deep_loss_alert_threshold_pct()
        pct = live.get("pct_of_max_loss")
        breached = pct is not None and pct >= threshold
        last_breached = int(t.get("deep_loss_alert_last_breached") or 0)
        if breached and not last_breached:
            log_alert_notification(
                "DEEP_LOSS",
                f"{t.get('symbol','').upper()} deep loss ({pct:.0f}% of max loss)",
                f"Unrealised loss is {pct:.0f}% of max loss (threshold {threshold:.0f}%) — "
                f"past this point recovery to profit is rare",
                symbol=(t.get("symbol") or "").upper(),
                trade_id=t.get("id"),
                severity="warn",
                source="journal",
                metadata={"trade_id": t.get("id"), "symbol": t.get("symbol"), "pct_of_max_loss": pct,
                          "threshold": threshold, "unrealised_pnl": live.get("unrealised_pnl"),
                          "max_loss": live.get("max_loss"), "trade_type": t.get("trade_type")},
            )
            _send_trade_deep_loss_telegram(t, live, threshold)
        con = _conn()
        try:
            con.execute(
                "UPDATE trades SET deep_loss_alert_last_breached=?, deep_loss_alert_last_sent_at=? WHERE id=?",
                (1 if breached else 0, datetime.now().isoformat(timespec='seconds') if breached else t.get("deep_loss_alert_last_sent_at"), t.get("id")),
            )
            con.commit()
        finally:
            con.close()
...
def _send_trade_deep_loss_telegram(t, live, threshold):
    """Send a Telegram alert for a trade crossing the deep-loss threshold."""
    try:
        t = _as_dict(t)
        from ..services.telegram_alerts import telegram_configured, send_telegram_message
    except Exception as e:
    ...
    parts = [
        f"🔻 Deep loss alert ({threshold:.0f}%+ of max loss)",
        f"Trade: #{t.get('id')} {str(t.get('symbol') or '').upper()}",
        f"Type: {t.get('trade_type') or '—'}",
        f"Loss: ${abs(pnl):.2f} ({pct:.0f}% of ${max_loss:.2f} max loss)" if pnl is not None and max_loss else "Loss: —",
        f"Spot: {float(live.get('spot')):.2f}" if live.get('spot') is not None else 'Spot: —',
        "Past this depth, recovery to profit is uncommon — worth a hard look at cutting losses now.",
    ]
    try:
        result = send_telegram_message("\n".join(parts))
        return {'configured': True, 'sent': 1 if result.get('ok') else 0, 'error': result.get('description') or result.get('error')}
    except Exception as e:
        return {'configured': True, 'sent': 0, 'error': str(e)}


def _scan_trade_deep_loss_alerts_once():
    """Same shape as _scan_trade_pnr_alerts_once: check every open
    position once, using each one's already-computed live P&L."""
    global _trade_deep_loss_last_run_at, _trade_deep_loss_last_result
    if not _deep_loss_alerts_enabled():
        _trade_deep_loss_last_run_at = datetime.now().isoformat(timespec='seconds')
        _trade_deep_loss_last_result = {'ok': True, 'enabled': False, 'triggered': 0, 'alerts': []}
        return
    con = _conn()
    try:
    ...
    for row in rows:
        try:
            ...
            before = row["deep_loss_alert_last_breached"] if "deep_loss_alert_last_breached" in row.keys() else 0
            threshold = _deep_loss_alert_threshold_pct()
            pct = live.get("pct_of_max_loss")
            newly_breached = pct is not None and pct >= threshold and not before
            _maybe_log_deep_loss_alert(row, live)
            if newly_breached:
                alerts.append({'id': row['id'], 'symbol': row['symbol'], 'pct_of_max_loss': pct,
                                'unrealised_pnl': live.get('unrealised_pnl')})
        except Exception:
            continue
    _trade_deep_loss_last_run_at = datetime.now().isoformat(timespec='seconds')
    _trade_deep_loss_last_result = {'ok': True, 'triggered': len(alerts), 'alerts': alerts}


_trade_deep_loss_last_run_at = None
_trade_deep_loss_last_result = {'ok': True, 'triggered': 0, 'alerts': []}


def _maybe_log_oi_shift_alert(t, live):
    """Independent OI-shift alert -- fires on its own, not only when a
    PNR breach happens to coincide. This is the early-warning half:
    price may still be comfortably clear of the short strikes while
    positioning has already shifted materially against them.

    Deduped per (trade, strike-set, direction) via the same
    alert_notifications table it writes to, checking whether an
    equivalent alert already went out for this trade in the last 24h --
    the caller runs every 60 seconds, and OI only updates once per
    fetch cycle, so without this the same shift would re-alert
    constantly for the rest of the day.
    """
    try:
        if not _is_open_option_trade(t) or not _pnr_alerts_enabled():
            return
        shift = _analyze_position_oi_shift(t, live)
        if not shift or not shift.get("notes"):
            return
        trade_id = _row_value(t, "id", None)
        sig = ",".join(sorted(f"{f['side']}{f['strike']}{f['direction']}" for f in shift["findings"]))
        con = _conn()
        try:
            recent = con.execute(
                """SELECT metadata FROM alert_notifications
                   WHERE alert_type='OI_SHIFT' AND trade_id=?
        ...
        for r in recent:
            try:
                if (_json.loads(r["metadata"] or "{}") or {}).get("sig") == sig:
                    return  # same shift already reported for this trade today
            except Exception:
                continue
        symbol = (_row_value(t, "symbol", "") or "").upper()
        building = any(f["direction"] == "building" for f in shift["findings"])
        log_alert_notification(
            "OI_SHIFT",
            f"{symbol} — OI shift at your short strike{'s' if len(shift['findings']) > 1 else ''}",
            "\n".join(shift["notes"]) +
            "\n\nThis is positional context, not a close/roll instruction on its own -- "
            "check it against the position's current action and P&L before acting.",
            symbol=symbol, trade_id=trade_id,
            severity="warn" if building else "info",
            source="journal",
            metadata={"trade_id": trade_id, "symbol": symbol, "sig": sig, "findings": shift["findings"]},
        )
    except Exception:
        pass
...
def _scan_trade_pnr_alerts_once():
    ...
    for row in rows:
        try:
            ...
            _maybe_log_trade_pnr_alert(row, live); _maybe_log_trade_sl_tp_alert(row, live)
            _maybe_log_oi_shift_alert(row, live)
            if before:
...
def start_trade_alert_watcher(interval_seconds=14400):
    """This check now runs from inside Signal Notifier's own background
    loop (see oiapp/scanners/signal_notifier.py), configurable from the
    Signal Notifier page instead of being a separate always-on watcher.
    Still registered here (job_registry only, no unified_scheduler
    dispatch) for Scheduler Hub visibility and a manual "Run Now"."""
    ...
    register_job(
        "trade_pnr_alerts", "Trade P&L/risk alerts (via Signal Notifier)",
        "Scans open journal positions for P&L / risk-level alerts. Runs automatically "
        "from Signal Notifier's background loop; configure enabled/interval at "
        "/signal-notifier. Run Now here triggers an extra ad-hoc check.",
        kind="interval", default_schedule={"interval_min": max(1, int((interval_seconds or 60) / 60))},
        group="Manual / One-Time", run_now_fn=_scan_trade_pnr_alerts_once,
    )
    return True
...
def _trade_width(t):
    ...
    if ss and ls:
        return abs(ss-ls)
    # Fall back to legs_json -- same reasoning as the frontend's width
    # calc: long_strike/short_strike used to get wiped to 0/NULL on every
    # edit regardless of the actual legs (fixed now, plus a one-time
    # repair for already-affected trades), so this covers any row that
    # still somehow has them missing.
    try:
        import json as _json
        legs = _json.loads(t.get("legs_json") or "[]")
        option_legs = [l for l in legs if l.get("option_type") not in (None, "stock") and l.get("strike")]
        if len(option_legs) >= 2:
            return abs(float(option_legs[0]["strike"]) - float(option_legs[1]["strike"]))
    except Exception:
        pass
    return 0
...
def _trade_probability_score(t, spot, dte, atr, pnr, pnr_upper,
    ...
    # Confluence: how many independent dimensions actually support this
    # trade right now, not just how much weight they added/subtracted
    # to the score above. Purely additive -- reuses the same inputs the
    # scoring logic above already has, doesn't touch that logic at all.
    # See oiapp/scanners/confluence.py for why this is tracked
    # separately from the weighted score.
    from ..scanners.confluence import compute_confluence
    _dte_agrees = 7 < dte <= 21 if dte is not None else None  # the "sweet spot"/"good window" tiers, not the imminent-expiry or too-long tiers
    _pnl_agrees = (pct_of_max >= 40) if pct_of_max is not None else None
    if pnr_breached:
        _pnr_agrees = False
    elif pnr and spot:
        if tt in ("PS", "PB"):
            _pnr_agrees = ((spot - pnr) / spot * 100) > 5
        elif tt in ("CS", "CB") and pnr_upper:
            _pnr_agrees = ((pnr_upper - spot) / spot * 100) > 5
        else:
            _pnr_agrees = None
    else:
        _pnr_agrees = None
    _atr_agrees = (atr / spot * 100 < 3) if (atr and spot) else None  # not "high ATR" tier
    _regime_agrees = None
    if rs_delta:
        _regime_agrees = (rs_delta > 0) if is_bull else (rs_delta < 0) if not is_bull else None
    _oi_agrees = None
    if oi_sig == "LONG_BUILDUP":
        _oi_agrees = is_bull
    elif oi_sig == "SHORT_BUILDUP":
        _oi_agrees = not is_bull

    confluence = compute_confluence({
        "dte": _dte_agrees, "pnl_momentum": _pnl_agrees, "pnr_proximity": _pnr_agrees,
        "atr_volatility": _atr_agrees, "regime_bias": _regime_agrees, "oi_shift": _oi_agrees,
    })
...
def _compute_live_pnl(trade, spot=None):
    ...
    pnr_breached = False
    pnr_status   = "SAFE"
    _pnr_side    = None  # initialized here, not only inside the branch below -- the
                         # roll-vs-close decision further down reads it unconditionally,
                         # so leaving it unset for non-option/no-spot trades would raise
                         # NameError instead of just falling through to "close"
    ...
    if dte == 0:
        ...
        # "CLOSE / ROLL" alone forces the person to work out which one --
        # exactly the decision that needs to be fast when a position is
        # already past half its max loss. Decide it here from facts
        # already computed above: rolling only genuinely helps if there's
        # enough time left for a new expiry to work AND the position is
        # under pressure from one side (a roll is a directional
        # adjustment, not a fix for a position that's simply wrong).
        # Under ~10 DTE a roll mostly buys gamma risk rather than
        # relief, so close instead.
        loss_amt = abs(unrealised_pnl)
        if dte >= 10 and _pnr_side in ("put", "call"):
            threatened = pnr if _pnr_side == "put" else pnr_upper
            direction = "down" if _pnr_side == "put" else "up"
            action = "ROLL"
            action_reason = (
                f"Loss ${loss_amt:.0f} > 50% of max (${max_loss:.0f}), but {dte} DTE left and pressure is "
                f"one-sided ({_pnr_side} side, spot moved {direction} toward ${threatened}). "
                f"Roll the {_pnr_side} side out in time and further {direction} — target a new short strike "
                f"beyond ${threatened}, ideally for a net credit. If it can't be rolled for a credit, close instead."
            )
        else:
            action = "CLOSE"
            reason_why = (
                f"only {dte} DTE left — a roll here mostly buys gamma risk, not relief"
                if dte < 10 else
                "both sides are under pressure, so there's no single side to roll away from"
            )
            action_reason = (
                f"Loss ${loss_amt:.0f} > 50% of max (${max_loss:.0f}) and {reason_why}. "
                f"Take the loss rather than adding risk to a position already past its stop."
            )
        urgency = "high"
    ...
    if unrealised_pnl is not None and unrealised_pnl < 0 and max_loss:
        pct_of_max_loss = round(abs(unrealised_pnl) / max_loss * 100, 1)
...
@journal_bp.route("/journal/trades/repair_strikes", methods=["POST"])
def repair_trade_strikes():
    """One-time repair for trades already saved with long_strike=0/
    short_strike=NULL -- update_trade used to unconditionally wipe these
    two columns on every single edit, regardless of what the legs
    actually were (fixed going forward; this fixes what's already
    broken). Re-derives both from legs_json wherever it has 2+ option
    legs with a clear sell/buy pair -- safe to run repeatedly, only
    touches rows that are actually missing this data."""
    import json as _json
    con = _conn()
    try:
        rows = con.execute(
            "SELECT id, trade_type, legs_json FROM trades "
            "WHERE (short_strike IS NULL OR long_strike IS NULL OR long_strike=0) "
            "AND legs_json IS NOT NULL AND legs_json != ''"
        ).fetchall()
        fixed = []
        skipped = []
        for row in rows:
            tid, trade_type, legs_json = row["id"], row["trade_type"], row["legs_json"]
            if trade_type == "IC":
                skipped.append(tid)  # IC uses put_sell/put_buy/call_sell/call_buy, not these two columns
                continue
            try:
                legs = _json.loads(legs_json or "[]")
            except Exception:
                skipped.append(tid)
                continue
            option_legs = [l for l in legs if l.get("option_type") not in (None, "stock")]
            if len(option_legs) < 2:
                skipped.append(tid)
                continue
            sell_leg = next((l for l in option_legs if l.get("side") == "sell"), None)
            buy_leg = next((l for l in option_legs if l.get("side") == "buy"), None)
            if not (sell_leg and sell_leg.get("strike") and buy_leg and buy_leg.get("strike")):
                skipped.append(tid)
                continue
            con.execute(
                "UPDATE trades SET long_strike=?, short_strike=? WHERE id=?",
                (float(buy_leg["strike"]), float(sell_leg["strike"]), tid),
            )
            fixed.append(tid)
        con.commit()
    finally:
        con.close()
    return jsonify({"ok": True, "fixed": fixed, "fixed_count": len(fixed), "skipped_count": len(skipped)})
...
def refresh_all():
    ...
    for t in trades:
        ...
        _maybe_log_trade_pnr_alert(t, live); _maybe_log_trade_sl_tp_alert(t, live)
...
@journal_bp.route("/journal/option_quote", methods=["GET"])
def journal_option_quote():
    """Live bid/ask/mid for one option contract, via Tastytrade -- used by
    the Add/Edit Trade forms' per-leg 'Fetch Price' button. Params:
    symbol, expiry (YYYY-MM-DD), strike, option_type (call/put).
    """
    symbol = (request.args.get("symbol") or "").strip()
    expiry = (request.args.get("expiry") or "").strip()
    strike = request.args.get("strike")
    option_type = (request.args.get("option_type") or "").strip()
    if not symbol or not expiry or not strike or not option_type:
        return jsonify({"error": "symbol, expiry, strike, and option_type are all required"}), 400
    try:
        strike = float(strike)
    except (TypeError, ValueError):
    ...
    try:
        ...
        return jsonify({"error": f"Realtime/Tastytrade module not available: {e}"}), 503

    result = get_option_quote(symbol, expiry, strike, option_type)
    if result.get("error"):
...
def add_trade():
    ...
    # Derive long_strike/short_strike from the legs array for non-IC
    # vertical spreads (PS, CS, and similar 2-option-leg trades). Without
    # this, these end up 0/NULL for anything entered through the modern
    # multi-leg UI (which sends `legs`, not top-level long_strike/
    # short_strike) -- the strikes still display correctly elsewhere
    # (that reads legs_json), but width/max-loss/max-gain calculations
    # everywhere else in the app (both here and in the frontend table)
    # read these two columns directly, so they silently break: max loss
    # shows blank, and any P&L math that depends on spread width is off.
    option_legs = [l for l in legs if l.get("option_type") not in (None, "stock")]
    derived_short_strike = None
    derived_long_strike = None
    if not is_ic and len(option_legs) >= 2:
        sell_leg = next((l for l in option_legs if l.get("side") == "sell"), None)
        buy_leg = next((l for l in option_legs if l.get("side") == "buy"), None)
        if sell_leg and sell_leg.get("strike"):
            derived_short_strike = float(sell_leg["strike"])
        if buy_leg and buy_leg.get("strike"):
            derived_long_strike = float(buy_leg["strike"])
    long_strike_val = derived_long_strike if derived_long_strike is not None else float(d.get("long_strike") or 0)
    short_strike_val = derived_short_strike if derived_short_strike is not None else (
    ...
    con.execute("""
        ...
        (entry_date,symbol,expiry,trade_type,trade_subtype,long_strike,short_strike,
         ...
         legs_json,num_legs,net_premium,stop_loss_price,target_price)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'OPEN',?,?,?,?,?,?,?,?,?,?,?)
    """, (
        ...
        num_legs,
        round(net_premium, 4),
        (float(d["stop_loss_price"]) if d.get("stop_loss_price") not in (None, "") else None),
        (float(d["target_price"]) if d.get("target_price") not in (None, "") else None),
    ))
...
def update_trade(tid):
    ...
    option_legs = [l for l in legs if l.get("option_type") not in (None, "stock")]
    long_strike_val = 0
    short_strike_val = None
    if not is_ic and len(option_legs) >= 2:
        sell_leg = next((l for l in option_legs if l.get("side") == "sell"), None)
        buy_leg = next((l for l in option_legs if l.get("side") == "buy"), None)
        if buy_leg and buy_leg.get("strike"):
            long_strike_val = float(buy_leg["strike"])
        if sell_leg and sell_leg.get("strike"):
            short_strike_val = float(sell_leg["strike"])
    ...
    con.execute("""UPDATE trades SET symbol=?,trade_type=?,expiry=?,entry_date=?,
        ...
        long_strike=?,short_strike=?,stop_loss_price=?,target_price=?,
        sl_tp_alert_last_state=NULL WHERE id=?""",
        (d.get("symbol"), d.get("trade_type"),
         ...
         legs_json, len(legs), round(net_premium,2), long_strike_val, short_strike_val,
         (float(d["stop_loss_price"]) if d.get("stop_loss_price") not in (None, "") else None),
         (float(d["target_price"]) if d.get("target_price") not in (None, "") else None),
         tid))
    con.commit(); con.close()
...
def start_health_alert_watcher(interval_seconds: int = 300):
    ...
    from ..services.job_registry import register_job
    register_job(
        "trade_health_alerts", "Trade health alerts (via Signal Notifier)",
        "Checks position health scores and pushes Telegram alerts on threshold changes. "
        "Runs automatically from Signal Notifier's background loop; configure "
        "enabled/interval at /signal-notifier. Run Now here triggers an extra ad-hoc check.",
        kind="interval", default_schedule={"interval_min": max(1, int((interval_seconds or 300) / 60))},
        group="Manual / One-Time", run_now_fn=_scan_health_alerts_and_notify,
    )
    return True
