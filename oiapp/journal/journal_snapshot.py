...
def ensure_snapshot_table():
    ...
    # Heal older trade_snapshot tables that predate columns added later
    # (CREATE TABLE IF NOT EXISTS above is a no-op on a table that already
    # exists, so a column added to the schema after someone's table was
    # first created never actually lands on their database -- this is
    # exactly what caused "table trade_snapshot has no column named
    # created_at" on a real, pre-existing database. Same
    # try/ALTER/except-pass pattern already used for the trades table
    # below; safe to re-run every startup since an existing column just
    # raises and gets swallowed.
    for sql in (
        "ALTER TABLE trade_snapshot ADD COLUMN created_at TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_spot REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN regime_monthly TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN regime_weekly TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN regime_daily TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_rs REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_iv REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_iv_rank REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_iv_pct REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_expected_move REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_pcr REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_call_wall REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_put_wall REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_gamma_flip REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_max_pain REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_net_gex REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_gex_ratio REAL",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_top_call_walls TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_top_put_walls TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_call_oi INTEGER",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_put_oi INTEGER",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_score INTEGER",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_grade TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_recommendation TEXT",
        "ALTER TABLE trade_snapshot ADD COLUMN entry_score_json TEXT",
    ):
        try:
            con.execute(sql)
        except Exception:
