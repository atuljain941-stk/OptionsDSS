...
let watchlists = [];
let savedScanners = [];
let scannerCatalog = {saved_scanners: [], builtin_scanners: [], functions: [], timeframes: [], indicators: []};
let selectedScannerId = null;
let rules = [];
let autocompleteItems = [];
let scannerPickerOpen = false;

function esc(s){return String(s ?? '').replace(/[&<>\"]/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m]));}
function tfLabel(tf){ return tf === '1m' ? '1m (monthly)' : tf; }
function quoteScannerName(name) {
  const s = String(name || '').trim();
  if (!s) return '""';
  if (/^[A-Za-z0-9_\-]+$/.test(s)) return s;
  return JSON.stringify(s);
}
function scannerInsertText(entry, mode = 'block') {
  const name = String(entry?.name || '').trim();
  const query = String(entry?.query_text || '').trim();
  if (mode === 'block' && query) {
    return `(${query})`;
  }
  return `scan(${quoteScannerName(name)})`;
}
...
function insertIntoQuery(text) {
  ...
  showAutocomplete();
  if (scannerPickerOpen) closeScannerPicker();
}
function renderScannerPicker(entries, typed = '') {
  const panel = document.getElementById('scanAutocomplete');
  if (!panel) return;
  const needle = String(typed || '').trim().toLowerCase();
  const filtered = (entries || []).filter(x => !needle || String(x.name || '').toLowerCase().includes(needle)).slice(0, 24);
  if (!filtered.length) {
  ...
  panel.innerHTML = filtered.map(x => {
    const block = scannerInsertText(x, 'block');
    const ref = scannerInsertText(x, 'ref');
    const preview = x.query_text ? esc(x.query_text).replace(/\n/g, '<br>') : esc(ref);
    return `
    <div class="autocomplete-item" data-block="${esc(block)}" data-ref="${esc(ref)}">
      <div style="min-width:0;flex:1">
        <b>${esc(x.name)}</b>
        <div class="mini">${esc(x.category || 'Scanner')} · ${esc(x.description || '')}</div>
        <div class="mini" style="margin-top:4px;max-height:70px;overflow:auto;white-space:normal">${preview}</div>
      </div>
      <div style="display:flex;gap:6px;flex-shrink:0">
        <button class="btn secondary" data-act="block">Block</button>
        <button class="btn secondary" data-act="ref">Ref</button>
      </div>
    </div>`;
  }).join('');
  panel.style.display = 'block';
  panel.querySelectorAll('[data-act="block"]').forEach(btn => btn.addEventListener('mousedown', (e) => {
    e.preventDefault();
    insertIntoQuery(btn.closest('.autocomplete-item')?.dataset.block || '');
    closeScannerPicker();
  }));
  panel.querySelectorAll('[data-act="ref"]').forEach(btn => btn.addEventListener('mousedown', (e) => {
    e.preventDefault();
    insertIntoQuery(btn.closest('.autocomplete-item')?.dataset.ref || '');
    closeScannerPicker();
  }));
}

function openScannerPicker() {
  scannerPickerOpen = true;
  showAutocomplete();
}

function closeScannerPicker() {
  scannerPickerOpen = false;
  const panel = document.getElementById('scanAutocomplete');
  if (panel) {
    panel.style.display = 'none';
    panel.innerHTML = '';
...
function showAutocomplete() {
  ...
  if (scannerPickerOpen) {
    renderScannerPicker(entries, '');
    return;
  }
  const text = ta.value;
  ...
  const typed = (m[1] || m[2] || m[3] || '').trim().toLowerCase();
  renderScannerPicker(entries, typed);
}
  ...
  for (const [cat, items] of Object.entries(groups)) {
    sections.push(`<div><div class="mini" style="margin:6px 0 4px;font-weight:700;color:var(--text)">${esc(cat)}</div>${items.map(x => {
      ...
      <div class="catalog-item">
        <div style="min-width:0;flex:1">
          <b>${esc(x.name)}</b>
          <div class="mini">${esc(x.description || x.query_text || '')}</div>
          <div class="mini" style="margin-top:4px;max-height:72px;overflow:auto;white-space:normal">${preview}</div>
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0">
          <button class="btn secondary" data-catalog-block="${esc(block)}">Block</button>
          <button class="btn secondary" data-catalog-ref="${esc(ref)}">Ref</button>
        </div>
      </div>`;
    }).join('')}</div>`);
  }
  box.innerHTML = sections.join('') || '<div class="small">No scanners found.</div>';
  box.querySelectorAll('[data-catalog-block]').forEach(btn => btn.addEventListener('click', () => insertIntoQuery(btn.dataset.catalogBlock || '')));
  box.querySelectorAll('[data-catalog-ref]').forEach(btn => btn.addEventListener('click', () => insertIntoQuery(btn.dataset.catalogRef || '')));
}
...
async function init() {
  ...
  q.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && e.shiftKey) {
      e.preventDefault();
      openScannerPicker();
      setStatus('Scanner picker opened');
    }
    if (e.key === 'Escape') {
      closeScannerPicker();
    }
