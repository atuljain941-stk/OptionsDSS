...
function moveColumn(idx,delta){
  ...
  if(window._lastScanData){window._lastScanData.result_columns=getResultColumns(); renderResults(window._lastScanData);}
  else renderResultShell();
  setStatus('Column order changed. Save template to keep this order.');
}
function sortIndicator(i){
  if(resultSort.index!==i) return '↕';
...
function sortableValue(v){
  ...
  const n=Number(raw.replace(/^\$/,'').replace(/%$/,'').replace(/,/g,''));
...
function sortedRows(items, cols){
  ...
  const idx=Number(resultSort.index);
  if(!Number.isInteger(idx) || idx<0 || idx>=cols.length) return rows;
  const col=cols[idx]; const dir=resultSort.dir==='desc' ? -1 : 1;
  return rows.sort((a,b)=>{
    const av=sortableValue(getRowValue(a,col));
    const bv=sortableValue(getRowValue(b,col));
    if(typeof av==='number' && typeof bv==='number') return (av-bv)*dir;
    return String(av).localeCompare(String(bv), undefined, {numeric:true, sensitivity:'base'})*dir;
  });
}
function renderSortableHead(head, cols){
  if(!head) return;
  if(resultSort.index!==null && (resultSort.index<0 || resultSort.index>=cols.length)) resultSort={index:null,dir:'asc'};
  head.innerHTML='<tr>'+cols.map((c,i)=>`<th><button class="sort-head" data-sort-col="${i}" title="Sort by ${esc(c.label)}">${esc(c.label)} <span class="sort-indicator">${sortIndicator(i)}</span></button></th>`).join('')+'</tr>';
}
function setResultSort(idx){
  idx=Number(idx);
  if(!Number.isInteger(idx)) return;
  if(resultSort.index===idx) resultSort.dir=resultSort.dir==='asc'?'desc':'asc';
  else resultSort={index:idx,dir:'asc'};
  if(window._lastScanData) renderResults(window._lastScanData);
  else renderResultShell();
}
function renderColumnList(){
  ...
  list.querySelectorAll('button[data-col-del]').forEach(btn=>btn.addEventListener('click',()=>{const idx=Number(btn.dataset.colDel); resultColumns.splice(idx,1); selectedColumnTemplateId=''; renderColumnTemplates(); renderColumnList(); if(window._lastScanData){window._lastScanData.result_columns=getResultColumns(); renderResults(window._lastScanData);} else renderResultShell(); setStatus('Column removed. Save template to keep this change.');}));
  list.querySelectorAll('.column-chip[draggable="true"]').forEach(chip=>{
    ...
    chip.addEventListener('drop',e=>{
      ...
      if(Number.isFinite(from)&&Number.isFinite(to)&&from!==to){
        ...
        selectedColumnTemplateId=''; renderColumnTemplates(); renderColumnList();
        if(window._lastScanData){window._lastScanData.result_columns=getResultColumns(); renderResults(window._lastScanData);} else renderResultShell();
        setStatus('Column order changed. Save template to keep this order.');
      }
    });
  });
...
function applyColumnTemplate(id){
  ...
  renderColumnTemplates(); renderColumnList(); setStatus(t?`Applied columns: ${t.name}`:'Using default columns');
}
function addColumn(label,expr,fmt){
  ...
  selectedColumnTemplateId=''; renderColumnTemplates(); renderColumnList(); setStatus(`Added column: ${meta.label||expr}`);
}
...
async function deleteColumnTemplate(){
  ...
  columnTemplates=d.templates||[]; selectedColumnTemplateId=''; resultColumns=defaultColumns(); renderColumnTemplates(); renderColumnList(); setStatus('Column template deleted');
}
...
function renderResults(d){
  ...
  if(head) head.innerHTML=resultHeaderHtml(cols);
  if(!items.length){
    if(body) body.innerHTML=`<tr><td colspan="${cols.length}" class="small">No matches.</td></tr>`;
    if(summary){summary.style.display='block'; summary.innerHTML='No symbols matched the current query.';}
    return;
  }
