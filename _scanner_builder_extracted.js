...
function showAutocomplete(forceAll = false) {
  ...
  const typed = forceAll ? '' : (m[1] || '').trim().toLowerCase();
  const entries = allScannerEntries().filter(x => !typed || String(x.name || '').toLowerCase().includes(typed)).slice(0, 24);
  if (!entries.length) {
    panel.style.display = 'none';
    panel.innerHTML = '';
    autocompleteItems = [];
  ...
  panel.innerHTML = entries.map((x, idx) => `
    <div class="autocomplete-item ${idx === autocompleteIndex ? 'selected' : ''}" data-idx="${idx}">
      <div style="min-width:0">
        <b>${esc(x.name)}</b>
        <div class="mini">${esc(x.category || 'Scanner')} · ${esc(scannerSnippet(x))}</div>
      </div>
  ...
  panel.querySelectorAll('button[data-action]').forEach(btn => btn.addEventListener('mousedown', (e) => {
    ...
    if (!item) return;
    if (btn.dataset.action === 'ref') insertScannerCall(item.name);
    ...
    sections.push(`<div><div class="mini" style="margin:6px 0 4px;font-weight:700;color:var(--text)">${esc(cat)}</div>${items.map(x => `
      <div class="catalog-item">
        <div style="min-width:0">
          ...
          <div class="mini" style="margin-top:4px"><code>${esc(scannerSnippet(x))}</code></div>
        </div>
